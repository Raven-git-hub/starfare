...
const { resolveManifest, usedSpace, copyManifestLine } = require('./manifest.js');
// The chained-route execution (the automation layer, transport-model.md §11.2) reuses three helpers
// from actions.js: the single-leg trip builder (the "next leg goes"), the own-Outpost-at-a-craft
// lookup (the arrival's dock-enqueue path), and the { anchor, action? } waypoint copy. actions.js does
// NOT require tick.js, so this one-directional edge is cycle-free.
const { buildSingleLegTrip, ownedOutpostAtCraft } = require('./actions.js');
...
// three helpers below are that execution. `advanceRoute` is the ONE FUNNEL both tick hooks call to
// "make the next leg go"; the two hooks (stepVehicleArrivals, stepOutpostDocks) are where a routed
// craft advances its route instead of simply landing / simply resolving. It stays event-driven — a
// per-arrival / per-turnaround-completion step in fixed id order — so there is no per-tick per-craft
// movement loop and determinism (§15.5 invariant 9) is preserved.
...
//
// A ROUTED craft (one carrying a `route`, the automation layer §11.2) lands the same way — but its
// trip was ONE LEG of a chained route, so after landing it resolves the arrived waypoint's action and
// advances to the next leg (`resolveRouteArrival` → `advanceRoute`). An outpost action instead queues
// and advances at turnaround, in stepOutpostDocks. This stays event-driven (per-arrival), so the
// determinism / no-movement-loop contract above holds for a routed craft too.
//
...
function stepVehicleArrivals(state, _actions) {
  ...
  for (const guild of state.guilds || []) {
    ...
    for (const craft of due) {
      ...
      delete craft.trip;
      // A ROUTED craft (the automation layer, §11.2) does not simply land — this leg was one leg of a
      // chained route, so resolve the arrived waypoint's action and advance to the next leg (an outpost
      // action instead queues and advances at turnaround, in stepOutpostDocks). A plain (route-less)
      // craft has just landed above; nothing more.
      if (craft.route) resolveRouteArrival(state, guild, craft, thisTick);
    }
  }
  return state;
}
...
// for the dock logic — a coherent arrive-then-dock order. The ROUTE-EMBEDDED auto-manifest (the
// automation layer, §11.2) now USES that adjacency directly: a routed craft arriving at an owned
// Outpost enqueues its waypoint action ON ARRIVAL in stepVehicleArrivals, and this step, running next
// the SAME tick, promotes it into a slot with no reordering. (A manual `transferCargo` still enqueues
// between ticks.) When a routed craft's slot resolves here, this step ADVANCES its route — the
// turnaround was the pause, completion is when the next leg goes.
//
