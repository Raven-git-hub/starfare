...
// ordered list of `{ dir: 'load' | 'unload', good, qty }` lines that fills and empties a craft's
// hold against a store; resolving it moves goods between the two. This file is that move, and NOTHING
// ELSE spells it — the instant-at-a-SYSTEM path (sim/actions.js `transferCargo`) and the
// at-completion-in-a-SLOT path (sim/tick.js the Outpost dock step) both call in here, so a system
// transfer and an Outpost transfer can never drift.
//
...
// `min(qty, source holds, destination's remaining space)` against the LIVE running totals, so it is
...
function resolveManifest(hold, capacity, store, manifest) {
  ...
  for (const line of manifest) {
    ...
    move(line.good, Math.min(line.qty, hold[line.good] || 0, byRoom), false);
  }
  ...
  for (const line of manifest) {
    ...
    move(line.good, Math.min(line.qty, store.get(line.good), bySpace), true);
  }
}

module.exports = { resolveManifest, usedSpace };
