...
test('an under-injected commitment BREACHES at the boundary (status flag only, no FEE)', () => {
  ...
  // equity offered ⇒ the owner keeps all of it. ⤳ 26-09-26: titanium's own (T1) base, 1.
  const sale = 8 * basePriceFor('titanium');
...
//
// 23 OF THE 27 HASHES IN THIS FILE WERE RE-PINNED, AND THAT IS THE INTENDED PER-TIER RESEED.
// A fresh galaxy now seeds each good at its own tier's base (T1 1, T2 10, T3 100) instead of a
// flat 10, and clamps it to its tier's band. So in both runs every T1 and T3 price row moved
// (T2 goods sit at 10 either way), and with them the quote-lock ring and the price history.
// Raw deuterium's row did NOT move: it is out of the tier system (design.md §8) and keeps its
// pre-slice 10 / 2 / 200 band (ruled 26-09-26, the same day). These hashes were re-pinned a
// second time for that follow-up (they had briefly carried deuterium at the Tier-1 base of 1).
// Proven in the build session by replaying both runs on both engines: the follow-up's ONLY
// delta was deuterium's `prices`, `priceRing` and `priceHistory` rows, and those rows are
// byte-identical to the pre-slice engine's.
//
//   - GOLDEN_UNLICENSED, the one hash with the prices STRIPPED on the run that sells nothing,
//     did NOT move. That is the proof: this slice reached that run through its prices alone.
//   - The COMMITTED run's scaffold sale is paid at titanium's and carbon_products' POSTED
//     prices, which now rest near their T1 base of 1 instead of 10. So beyond the price data,
//     exactly three fields moved: the guild's `credits` (1,153 → 120 over the 40 ticks), its
//     `lastSyndicateSale` record, and the Syndicate `ledger` (the mirror, so invariant 2 still
//     closes). No fuel, grant, reputation or production field moved — checked in the build
//     session by diffing this run's final state under the old and new bands. That run has no
//     price-stripped golden, so every one of its hashes moved.
//   - Every value a test asserts EQUAL to, the `*_BEFORE_FUEL_FLOWS` ones included, was
//     re-pinned to what the same run and the same strip produce under the new bands.
//   - The three `COMMITTED_*_BEFORE_CONTROLLER` records were NOT re-pinned, exactly as the
//     yield-tier slice left them: they are what the pre-controller engine produced.
//
...
// 24 PREVIOUS values a test asserts equal to (all but the three BEFORE_CONTROLLER records)
// reproduced byte for byte. The previous values are in git at commit 2732d9f (the ruling
// commit, which changed no code).
const GOLDEN_UNLICENSED = '682e42e0dd758ce523fea882f6560707802cdd7e7b4def794f323430a4cfcff5';
const GOLDEN_UNLICENSED_WITH_PRICES = '1cbfa6632e16e0c96b63e6b34aed82f6f8fe242cffeebdd8ae352358e7caf00c';
const GOLDEN_COMMITTED_WITH_PRICES = 'c44ef9447f7a86ffbde3b4be96b2e4607c5f0404def82753963371b7fec7fcf1';
...
const COMMITTED_WITH_PRICES_BEFORE_FUEL_FLOWS = 'f8d3234b3ccabd338803fedc3ab303e2f5b41785a64477b35156047d9789fc08';
const GOLDEN_UNLICENSED_WITH_HISTORY = '25fda323840faf48c08e9007e6faec196397c2821ea2ab551da889ea4986c5dd';
const GOLDEN_COMMITTED_WITH_HISTORY = 'f54321f591e8471df1da76426194f92c83bff227b471e1c017fb8fc6735e0180';
const COMMITTED_WITH_HISTORY_BEFORE_FUEL_FLOWS = '0d641871c42e5bef9daa587e0c83ca8b44ff57a6073aecfb6fc67a0ed9ee1d02';
const GOLDEN_UNLICENSED_WITH_PRICE_HISTORY = '28997e29cd853e11feda1b58de1d09ba33e8b0f59bf7a8f6b5e5520dcef4636e';
const GOLDEN_COMMITTED_WITH_PRICE_HISTORY = '203affa9d489315eed2e67c2bd07c14562bd5ece9f6842c1b0cbfb5fa487a818';
const COMMITTED_WITH_PRICE_HISTORY_BEFORE_FUEL_FLOWS = '1517a340a4857f6255f2d1dcaa438bcf7fc2cffe20ca20d337f77f56c21e548a';
...
const GOLDEN_UNLICENSED_WITH_FUEL_PRICE = '71dc9058a7fdb5f44cddcfa9a832712777b74e4477a1cea308410db74a77c130';
// (its pre-controller value was 25698f31…, kept in the note above as one of the three)
const GOLDEN_COMMITTED_WITH_FUEL_PRICE = '561bf3f52a4c1e967e6bb334c4fb51c8466fdca0f7434b0fb99294e3d694b251';
...
const GOLDEN_UNLICENSED_WITH_CONTROLLER = '853048ef94191763124e14955f8519fc2180e20ed4acea878f2baee169df422d';
const GOLDEN_COMMITTED_WITH_CONTROLLER = '8134cee9cfc429139dae4390e1fcf6698f54992a3d548a42c80fa0d75c61d01f';
...
const GOLDEN_COMMITTED_WITH_GUILD_HALL = '473dbe9df2f5f3fdf2476dbc5dcec2ffa5105b8b43b8fd8f152fe290a32e90c9';
...
const GOLDEN_UNLICENSED_WITH_PRICE_RING = '2838a934232021bf2d6a49e6af3f6d822a68623fff3eb5cb1b3a71af9672de9c';
const GOLDEN_COMMITTED_WITH_PRICE_RING = '8d48678796fa39514e50652cf2b953c6a2c1b294c3ee4ada41c89ccc96189b06';

// ── THE EXPECTED-FUEL-CHANGE GAUGE (06-09-26 — docs/guild-hall.md §2.1, Slice D) ──
//
// SAME STORY AS THE GUILD HALL A/B/C SLICE: the COMMITTED run's FULL hash moved; every
// stripped golden above, and every UNLICENSED golden, did NOT — and that split is the proof.
// The slice adds ONE serialized key, `entitlement`, to the `lastFuelGrant` record (this
// cycle's fuel-credit entitlement, `grantFor` at the boundary), stamped under the SAME
// `desired > 0` sparsity the record already uses. (`predictedGrant` is snapshot-only — a
// derived field — so it moves no golden at all.) Because it is an ADDED KEY, the ordinary
// strip works: `withoutFuelEntitlement` peels it back out and `GOLDEN_COMMITTED_WITH_PRICE_RING`
// returns byte-for-byte — this slice touched no grant, no hoard, no pool, no price, no window,
// no reputation, only this one record field. The new full hash is pinned below.
//
// WHY THE COMMITTED RUN GAINED IT AND THE UNLICENSED ONE DID NOT is exactly as before: the
// committed run crosses ten boundaries and is due a grant at each, so its record carries the
...
const GOLDEN_COMMITTED_WITH_FUEL_ENTITLEMENT = '96693a5828797f783677122c55e3269133a1d7e723332a9740f5b899ada98e2f';

// ── THE FUEL-BURN HISTORY (07-09-26 — docs/guild-hall.md, the fuel-burn-history subsection) ──
//
// SAME STORY AS THE GUILD HALL A/B/C AND SLICE D SLICES: the COMMITTED run's FULL hash moved;
// every stripped golden above, and every UNLICENSED golden, did NOT — and that split is the proof.
// The slice adds three per-guild fields, but only ONE reaches this run's serialized state: the
// `fuelBurnHistory` ring, pushed at every boundary the guild is DUE a grant (`desired > 0`), under
// the same sparsity the grant record uses. The other two — `deuteriumFuelAtCycleStart` and the
// `fuelBurnedThisCycle` accumulator — are BOTH omit-when-0, and this run holds no contraband and
// issues no SELL/BUY (so nothing burns), so neither key is ever minted. Because the ring is an
// ADDED KEY, the ordinary strip works: `withoutFuelBurnHistory` peels it and
// GOLDEN_COMMITTED_WITH_FUEL_ENTITLEMENT returns byte-for-byte — this slice touched no grant, no
// hoard, no pool, no price, no window, no reputation, only this one new ring. The new full hash is
// pinned below. Every `granted` in the ring is the PREVIOUS boundary's grant (the closing cycle's
// funding), never the new one — the load-bearing boundary order, asserted directly below.
//
// WHY THE COMMITTED RUN GAINED IT AND THE UNLICENSED ONE DID NOT is exactly as before: the
...
const GOLDEN_COMMITTED_WITH_FUEL_BURN_HISTORY = '921b26e37fe4e09a649d55403e6696526dd812a6d5f3519f6ac4c04bbdba813e';
...
const GOLDEN_UNLICENSED_WITH_FUEL_PRICE_HISTORY = 'abd2873feffbfdc700420b0cba50448d8a87d99a626c170e9d899111ca38e587';
const GOLDEN_COMMITTED_WITH_FUEL_PRICE_HISTORY = 'a39d9385eae3cf88b060d0ce8fd10a560ed9f461debc8c0ea7bb215cd2f19fe1';
...
const GOLDEN_UNLICENSED_WITH_GUILD_HALL = 'ae02be97a48753255007d9c3ab35524836436d116aac39485d437434e0320bef';
...
// The state minus the expected-fuel-change gauge's serialized field (06-09-26, Slice D,
// docs/guild-hall.md §2.1) — everything the hashes above covered before it. An ADDED key
// INSIDE `lastFuelGrant`, so the ordinary strip works: per guild, drop `entitlement` from the
// grant record while keeping the rest (a guild with no record is left untouched). Peel it and
// GOLDEN_COMMITTED_WITH_PRICE_RING returns byte-for-byte — the proof it moved nothing else.
// (`predictedGrant` is snapshot-only and never reaches serialized state, so nothing to strip.)
const withoutFuelEntitlement = (state) => ({
  ...state,
  guilds: (state.guilds || []).map((g) => {
    if (!g.lastFuelGrant) return g;
    const { entitlement, ...grant } = g.lastFuelGrant;
    return { ...g, lastFuelGrant: grant };
  }),
});
// The state minus every guild's fuel-burn history ring (07-09-26, the fuel-burn-history slice,
// sim/fuel-burn-history.js) — everything the hashes above covered before it. An ADDED per-guild
// key, so the ordinary strip works: drop `fuelBurnHistory` and leave the rest of the guild row
// in place, so a change ANYWHERE else still fails. Peel it and GOLDEN_COMMITTED_WITH_FUEL_ENTITLEMENT
// returns byte-for-byte — the proof this slice moved nothing else. (The other two new fields —
// `deuteriumFuelAtCycleStart` and `fuelBurnedThisCycle` — are BOTH omit-when-0 and neither run
// here holds contraband or burns any fuel, so neither key is ever minted and there is nothing to
// strip: only the burn-history ring, pushed at every boundary a grant is due, moves this run.)
const withoutFuelBurnHistory = (state) => ({
  ...state,
...
test('NO-OP PROOF: an unlicensed run (no scaffold action) is byte-identical to pre-change HEAD', () => {
  ...
  // And the fuel-burn-history no-op (07-09-26): crossing no boundary, this run pushed no entry,
  // and issuing no SELL/BUY it burned nothing — so none of the three new fields was ever minted.
  assert.equal(s.guilds[0].fuelBurnHistory, undefined, 'the burn-history ring was never pushed off a boundary');
  assert.equal(s.guilds[0].fuelBurnedThisCycle, undefined, 'and nothing burned, so the accumulator key was never minted');
  assert.equal(s.guilds[0].deuteriumFuelAtCycleStart, undefined, 'and no contraband, so no red baseline was stamped');
});
...
test('a committed run seeded via createVenture (not the action) is pinned, and now PAYS', () => {
  ...
  // SLICE D (06-09-26): `withoutFuelEntitlement` joins it as the outermost strip, so the new
  ...
  assert.equal(typeof gh.lastFuelGrant.entitlement, 'number', 'Slice D: the grant carries this cycle\'s fuel-credit entitlement');
  // The fuel-burn history ring: one entry per boundary (ten), and each entry's `granted` is the
  // PREVIOUS boundary's grant, never the new one — the load-bearing boundary order. The first is 0
  // (no grant preceded the first boundary); the second is the grant the FIRST boundary recorded.
  assert.equal(gh.fuelBurnHistory.length, 10, 'fuel-burn history: one entry per boundary, ten boundaries');
  assert.equal(gh.fuelBurnHistory[0].granted, 0, 'the first cycle was funded by no prior grant');
  assert.ok(gh.fuelBurnHistory[1].granted > 0, 'the second cycle was funded by the first boundary\'s (non-zero) grant');
