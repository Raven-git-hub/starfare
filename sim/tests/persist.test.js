...
//
// WAYSTATION REBALANCE (02-09-26) — ALL NINE GOLDENS BELOW RE-PINNED AGAIN. The §16
// step 7 segmented scheme regenerated data/seed.json (waystations 9 → 96), which BOTH
// reshuffled the archetype layout — the derived home moved sys_0004/pl_00009 →
// sys_0006/pl_00015 — AND grew the zero-state's Syndicate claim set from 9 to 96
// waystation claims. So this sequence's state gained 87 claims and changed its home
// ids; every hash moved. Proven seed-shift-only, NOT an engine change: no sim engine
// file changed this slice, and removing the 87 added waystation claims (out_10..out_96)
// then id-swapping sys_0006→sys_0004 / pl_00015→pl_00009 reproduces each pre-slice
// golden byte-for-byte. The strip-and-prove tests below (id-independent arithmetic)
// still hold, which is the check.
//
...
// GUILD HALL ENGINE FIELDS (02-09-26 — docs/guild-hall.md §4, slices A/B/C): the six goldens
// below (through GOLDEN_HASH_WITH_AVG_DRAW) were UNCHANGED under A/B/C, because those fields were
// stamped ONLY at a cycle boundary and this 2-tick sequence crosses none.
//
// SLICE A′ (03-09-26 — §4): `fuelHoardAtCycleStart` is now stamped at CREATION too (= the guild's
// opening `fuelHoard`). This canonical sequence FOUNDS A GUILD, which opens on GUILD_STARTING_FUEL,
// so the founded guild gains `fuelHoardAtCycleStart: 500` from birth — and, crossing no boundary,
// it is never re-stamped. So the FULL hash moved (re-pinned as GOLDEN_HASH_WITH_GUILD_HALL below).
// The move is an ADDED KEY, so the ordinary strip works: the six goldens above are ALL UNCHANGED
// and are now asserted with `withoutGuildHall` applied, which is the whole proof — A′ added one
// per-guild field at creation and altered NOTHING else about this sequence, byte for byte.
// ── THE RESOURCE YIELD TIERS (24-09-26 — design.md §2 "Resource Yield Tiers & the Homeworld
// Production Floor") ──
//
// 14 OF THE 15 PINNED HASHES IN THIS FILE WERE RE-PINNED. The slice replaced the uniform MINE_BASELINE
// of 5 with the tiered yields (titanium 160, …); the baseline is the price engine's CAPACITY
// denominator, so every posted price in the canonical sequence moved, and with it every hash
// that keeps the price block (or its history / ring / at-rest vocabulary) present. The
// sequence's own mine is untouched — it still names its explicit rate — so the table reached
// it through the prices and nothing else: GOLDEN_HASH, the one with the prices STRIPPED, is
// the one that did NOT move.
//
// The historical `*_BEFORE_*` values were re-pinned to what the same strips produce under the
// new table, so their tests still prove each earlier slice's delta is exactly its own fields —
// now measured against today's yields rather than the uniform 5 they were first recorded under.
//
...
// change of the slice in place, all 15 PREVIOUS values reproduced byte for byte. The previous
...
const GOLDEN_HASH_WITH_PRICES = '72fc99269952ace4465c8af45d1fcfd08206a23ce8e0fdab1fc7ead5622fa9ec';
const GOLDEN_HASH_WITH_HISTORY = '0e903d0f7e74e1864eb979a6c2f692354c9976892ba7279c7b27b0be71549099';
const GOLDEN_HASH_WITH_ASSETS = '142ee9a58737818960266b6eaa1dff6062809c2517b841c77c67871cc0aafb10';
...
const GOLDEN_HASH_BEFORE_POOL_SEED = 'e2acaac7315d1eaa5d07628ca1b95e9ae9fa032c9eaadcc80b2f854a1be76fe6';
...
const GOLDEN_HASH_BEFORE_ENDOWMENT = 'df981e5b9897dcf34188af26b4594d8702e39a16b428425b0885f926f8e80d2a';
...
const GOLDEN_HASH_BEFORE_FUEL_GRANT = 'fb6798f5be8d9b93e7b089c950e104925281b252a7a66ff604739289d422e0e9';
...
const GOLDEN_HASH_WITH_FUEL_PRICE = 'd83c1bd06e38fb80ffa3dfaf66f8a58a9a2b194e17c051c1f67ec7170b8cbe41';
...
const GOLDEN_HASH_WITH_AVG_DRAW = 'df7bc855be822c1e5ca97e073b419fc32e1b60df398afed0f9c883481ae59141';
...
const GOLDEN_HASH_WITH_GUILD_HALL = 'b3166a5965775e5d8233e4d57e7bef1c91852c0da9b24948670357f8e2be89a1';
...
// is caught too.
const GOLDEN_HASH_WITH_PRICE_RING = 'be93e3a9f869dd04ef157b3822e8552808309934aff3756b6659209972074879';
...
// at founding, the Fuel Δ gauge's baseline before its first boundary. It is ADDED serialized
// state on a FOUNDED guild, so this sequence (which founds `player-guild`) legitimately moved
// its FULL hash, and the ordinary strip works: `withoutFoundingEntitlement` takes it back out
// and GOLDEN_HASH_WITH_PRICE_RING returns byte-for-byte — the whole delta proof. For THIS
// founding the baseline is 60 (home system GP 200 × modifier 1.0 × BASE_GRANT_PER_GP 0.3, the
...
const GOLDEN_HASH_WITH_FOUNDING_ENTITLEMENT = '170d9f833b3ecc398cba4d7a4dc610afcf6216afcbf6c08320a33f6261c681be';
...
const GOLDEN_HASH_WITH_FUEL_PRICE_HISTORY = '6f88bd306ec50511bfe3dccb03907760df887c23aaccdc0d248a052059eee6a3';
...
// hash moved: galacticSupply gained 26 zero rows, and prices/priceRing each gained 26
// base-price rows (no venture here manufactures a module, so every new good is at rest).
// ADDED VOCABULARY, not an added field — but the strip idiom still applies: `withoutTier3`
// removes exactly those 26 keys from prices, priceRing, priceHistory and galacticSupply,
// and GOLDEN_HASH_WITH_FUEL_PRICE_HISTORY returns byte-for-byte (asserted above), which is
...
const GOLDEN_HASH_WITH_TIER3_CATALOG = 'cd7515a4c2832b56eb1e4d58ab3147e4ed0503a7919d4b672ffa1de5554922f1';
...
const GOLDEN_HASH_WITH_ASSET_SYSTEMID = 'b39e9f2337063b63e6c4d2c8e292d42a68cafcc70bf96b008d3283d1d320c617';
...
// The state minus the Guild Hall engine fields (docs/guild-hall.md §4). Per guild, drop
// `fuelHoardAtCycleStart` (Slice A/A′) and `modifierHistory` (Slice C), and `modifier` from
// inside `lastFuelGrant` (Slice B) while keeping the rest of the record — the same strip
// commitment-scaffold.test.js uses. For THIS 2-tick sequence only the founding-stamped
// `fuelHoardAtCycleStart` is present, so this strip removes exactly that one added key.
...
// …and the Fuel Δ founding baseline (Slice D′) — an added per-guild key on a FOUNDED guild, so
// the ordinary strip recovers every pre-slice golden byte-for-byte.
const withoutFoundingEntitlement = (state) => ({
  ...state,
...
// TIER-3 CATALOG (2.1a — docs/asset-recipes.md). The slice added 26 goods to the
// priced/stockpile vocabulary: the new Tier-2 good `luminite_glass` + the 25 Tier-3
// modules. On a run that MANUFACTURES no module (every sequence here does), they enter
// the serialized state ONLY as "the new vocabulary at rest": a price row (base 10, since
// productionCapacity is 0 → priceTarget rests it at base), a galactic-supply zero row, a
// quote-lock ring entry, and — on runs past the first fine bucket — a priceHistory ring
// sampling that flat base. NO existing good's price, stock or production number moves.
//
// So the FULL hash of every sequence moved (galacticSupply, which no earlier strip
// touches, gained 26 zero rows; prices/priceRing/priceHistory gained 26 rows each). This
// strip removes exactly those 26 keys from exactly those four places, and the remainder
// must hash to the PRE-2.1a golden byte-for-byte — the whole delta proof, the same idiom
// withoutPrices/withoutAssets use, now for an added VOCABULARY rather than an added field.
...
// ASSET SYSTEMID (2.1b — design.md §4/§15.4, 12-09-26). Every asset now carries a
// `systemId` — its physical location, stamped at founding to the guild's home system.
// This canonical sequence founds a guild (25 idle starter machines) and establishes a
// mine (occupying one), so 25 asset rows each gained one key, moving the FULL hash. It
// is an ADDED FIELD, so the ordinary strip works: drop `systemId` from every asset row
// (leaving `withoutAssets` — which removes the whole array — untouched, since the runs
// that use it never see this key) and the pre-2.1b golden returns byte-for-byte, the
// whole delta proof. On a guild with no assets this is a no-op, so a run that founds
// nothing is byte-identical with nothing stripped.
const withoutAssetSystemId = (state) => ({
  ...state,
  guilds: (state.guilds || []).map((g) => (
    g.assets
      ? { ...g, assets: g.assets.map((a) => { const { systemId, ...rest } = a; return rest; }) }
      : g
  )),
});
...
test('no-op proof: pure engine path (persistence OFF) matches the golden hash', () => {
  ...
  // 2.1a: the Tier-3 module catalog added 26 goods to the priced/stockpile vocabulary,
  // so galacticSupply, prices, priceRing (and, on longer runs, priceHistory) each gained
  // 26 "at rest" rows and this sequence's FULL hash moved. `withoutTier3` is the newest,
  // OUTERMOST strip now — peel the 26 new-vocabulary rows and every earlier golden returns
  // byte-for-byte, the proof the catalog's only footprint here is the new goods sitting
  // idle. `bare` folds it in with the rest.
  const bare = (x) => withoutTier3(withoutFuelPriceHistory(withoutFoundingEntitlement(withoutPriceRing(withoutGuildHall(withoutAvgDraw(withoutFuelPrice(x)))))));
  assert.equal(hashState(bare(withoutAssets(withoutHistory(withoutPrices(s))))), GOLDEN_HASH, 'everything but the price block, the history buffer, the assets, the two reserve fields, the Guild Hall field, the price ring, the founding baseline, the fuel-price history and the Tier-3 vocabulary is byte-identical to pre-price-engine HEAD');
  ...
  // 2.1b: every asset gained a `systemId`, so every hash below that keeps the assets present
  // (from here down) moved. `withoutAssetSystemId` is the newest, OUTERMOST strip now — peel
  // that one key off each asset row and every earlier golden returns byte-for-byte, the proof
  // its only footprint here is the location stamped on the 25 idle machines + the one deployed.
  assert.equal(hashState(bare(withoutAssetSystemId(s))), GOLDEN_HASH_WITH_ASSETS, 'and with the assets back in (their systemId stripped), the ONLY delta from the pre-5b-i engine is the two reserve fields');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutFoundingEntitlement(withoutPriceRing(withoutGuildHall(withoutAvgDraw(s)))))))), GOLDEN_HASH_WITH_FUEL_PRICE, 'with the price back in, the ONLY delta from the pre-5b-ii engine is reserve.avgDraw');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutFoundingEntitlement(withoutPriceRing(withoutGuildHall(s))))))), GOLDEN_HASH_WITH_AVG_DRAW, 'and with the controller state back in, stripping the asset systemId, the Tier-3 vocabulary, the fuel-price history, the Guild Hall field, the price ring and the founding baseline returns the pre-Guild-Hall bytes');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutFoundingEntitlement(withoutPriceRing(s)))))), GOLDEN_HASH_WITH_GUILD_HALL, 'and with the Guild Hall field back in, stripping the asset systemId, the Tier-3 vocabulary, the fuel-price history, the price ring and the founding baseline returns the pre-quote-lock bytes');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutFoundingEntitlement(s))))), GOLDEN_HASH_WITH_PRICE_RING, 'and with the price ring back in, stripping the asset systemId, the Tier-3 vocabulary, the fuel-price history and the founding baseline returns the pre-Slice-D′ bytes');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(s)))), GOLDEN_HASH_WITH_FOUNDING_ENTITLEMENT, 'and with the founding baseline back in, stripping the asset systemId, the Tier-3 vocabulary and the fuel-price history returns the pre-fuel-price-history bytes');
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(s))), GOLDEN_HASH_WITH_FUEL_PRICE_HISTORY, 'and with the fuel-price history back in, stripping the asset systemId and the Tier-3 vocabulary returns the pre-2.1a bytes');
  assert.equal(hashState(withoutAssetSystemId(s)), GOLDEN_HASH_WITH_TIER3_CATALOG, 'and with the Tier-3 vocabulary back in, stripping only the asset systemId returns the pre-2.1b bytes — the whole systemId delta proof');
  assert.equal(hashState(s), GOLDEN_HASH_WITH_ASSET_SYSTEMID, 'and the full state — with a systemId on every asset — is pinned');
  ...
  // D′: the founding baseline is present from birth, so the strip above is a real proof — the
  // Fuel Δ gauge reads live (×1.00) from tick 0. 60 = home-system GP 200 × modifier 1.0 × 0.3.
  assert.equal(s.guilds[0].foundingEntitlement, 60, 'D′: the Fuel Δ founding baseline was stamped at founding');
...
test('no-op proof: subtract the founding fuel grant and the pre-slice golden comes back', () => {
  ...
  // …and the Fuel Δ founding baseline (Slice D′) — a later founding-stamped field this golden
  // predates. A pure additive key (not in the reputation sum), so a plain delete undoes it.
  delete ungranted.guilds[0].foundingEntitlement;
  ...
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutPriceRing(withoutGuildHall(withoutAvgDraw(withoutFuelPrice(ungranted)))))))), GOLDEN_HASH_BEFORE_FUEL_GRANT,
    'the fuel grant and the pool seed are the ONLY deltas the two fuel slices made to the canonical sequence, byte for byte (the later asset systemId, the A′ field, the price ring, the fuel-price history and the Tier-3 vocabulary stripped)');
});
...
test('no-op proof: un-seed the pool and the pre-slice-5a golden comes back', () => {
  ...
  delete unseeded.guilds[0].foundingEntitlement; // Slice D′, a later founding-stamped key this golden predates
  ...
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutPriceRing(withoutGuildHall(withoutAvgDraw(withoutFuelPrice(unseeded)))))))), GOLDEN_HASH_BEFORE_POOL_SEED,
    'the pool seed is the ONLY delta slice 5a made to this run, byte for byte (the later asset systemId, the A′ field, the price ring, the fuel-price history and the Tier-3 vocabulary stripped)');
});
...
test('no-op proof: un-endow the founding and the pre-endowment golden comes back', () => {
  ...
  assert.equal(hashState(withoutAssetSystemId(withoutTier3(withoutFuelPriceHistory(withoutPriceRing(withoutGuildHall(withoutAvgDraw(withoutFuelPrice(unendowed)))))))), GOLDEN_HASH_BEFORE_ENDOWMENT,
    'the endowment is the ONLY delta this slice made to the canonical sequence, byte for byte (the later asset systemId, the A′ field, the price ring, the fuel-price history and the Tier-3 vocabulary stripped)');
});
