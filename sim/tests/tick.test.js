...
test('production: a venture with no resourceType mines nothing and is not stamped', () => {
  const s = createState({
    guilds: [{ id: 'g1', credits: 0, fuelHoard: 0, ventures: [{ id: 'v1', ownerGuildId: 'g1', type: 'refinery', productionRate: 4 }] }],
    reserve: { reserveLevel: 0 },
    syndicate: { ledger: 0 },
  });
  const next = tick(s);
  assert.deepEqual(next.guilds[0].stockpiles, {}, 'no named good => nothing deposited');
  assert.equal(next.guilds[0].ventures[0].updatedAtTick, null, 'no mutation => no tick stamped');
});
