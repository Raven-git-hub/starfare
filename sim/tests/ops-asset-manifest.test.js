...
// single `<kind> asset` manifest row. If a future edit drops that branch, an asset delivery
// falls back through the goods loop to "No cargo listed." — this test fails loudly first.
// The snapshot half (each shipment carries `assetKind` + leg geometry) is proven in
// asset-purchase.test.js and transport-visibility.test.js.
...
test('rowHtml branches its manifest on sh.assetKind and names the asset', () => {
  // The manifest is chosen off `sh.assetKind`: an asset delivery names itself; a goods
  ...
  // The asset branch emits ONE .ops-manrow whose .g is `<kind> asset` — the kind plus the
  // literal word "asset" (`.ops-manrow .g` capitalizes it to "Miner Asset" / "Factory Asset").
  // No quantity/Nu span on the asset row.
  assert.match(
    html,
    /'<div class="ops-manrow"><span class="g">' \+ esc\(sh\.assetKind\) \+ ' asset<\/span><\/div>'/
  );
});
