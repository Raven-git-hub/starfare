...
test('establishing a mine seats it and it produces on the next tick', () => {
  ...
  const { state: next, results } = advance(s, [
    createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_1', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 }),
  ]);
...
test('two mines on two nodes sum their rates each tick', () => {
  let s = playerFounded();
  s = advance(s, [createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_1', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 })]).state; // +5 => 5
  s = advance(s, [createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_2', siteId: 'pl_00004_n02', resourceType: 'titanium', productionRate: 3 })]).state; // +5+3 => 13
...
test('a duplicate venture id is rejected', () => {
  let s = playerFounded();
  s = advance(s, [createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_1', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 })]).state;
  const v = validateAction(s, createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_1', siteId: 'pl_00004_n02', resourceType: 'titanium', productionRate: 5 }));
  assert.equal(v.valid, false);
  assert.match(v.reason, /already exists/);
});

test('a site that does not exist in the seed is rejected', () => {
  const v = validateAction(playerFounded(), createEstablishVentureAction({ guildId: 'player', ventureId: 'v', siteId: 'pl_99999_n99', resourceType: 'titanium', productionRate: 5 }));
  assert.equal(v.valid, false);
  assert.match(v.reason, /does not exist/);
});

test('a settlement slot is rejected (mining ventures need a resource node)', () => {
  const v = validateAction(playerFounded(), createEstablishVentureAction({ guildId: 'player', ventureId: 'v', siteId: 'pl_00004_s01', resourceType: 'titanium', productionRate: 5 }));
  assert.equal(v.valid, false);
  assert.match(v.reason, /not a resource node/);
});

test('a resourceType that does not match the node is rejected', () => {
  const v = validateAction(playerFounded(), createEstablishVentureAction({ guildId: 'player', ventureId: 'v', siteId: 'pl_00004_n01', resourceType: 'gold', productionRate: 5 }));
  assert.equal(v.valid, false);
...
test('an already-occupied node is rejected', () => {
  let s = playerFounded();
  s = advance(s, [createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_1', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 })]).state;
  const v = validateAction(s, createEstablishVentureAction({ guildId: 'player', ventureId: 'mine_2', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 }));
  assert.equal(v.valid, false);
...
test('a non-positive or non-integer productionRate is rejected', () => {
  ...
  for (const rate of [0, -3, 2.5]) {
    const v = validateAction(s, createEstablishVentureAction({ guildId: 'player', ventureId: 'v', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: rate }));
...
test('two ventures racing for the same node: first-valid-wins', () => {
  ...
  const { state: next, results } = intake(s, [
    createEstablishVentureAction({ guildId: 'player', ventureId: 'first', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 }),
    createEstablishVentureAction({ guildId: 'player', ventureId: 'second', siteId: 'pl_00004_n01', resourceType: 'titanium', productionRate: 5 }),
  ]);
  assert.equal(results[0].accepted, true);
