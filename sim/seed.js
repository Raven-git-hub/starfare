...
// --- The hex lattice (where a bare-hex vehicle location may sit) ------------

// hexToPixel(q, r, hexSize) -> the flat-top axial hex's world-space centre. The EXACT
// formula tools/generate_seed.js used to place systems and to enumerate the galaxy's
// hexes, copied verbatim so a consumer's lattice agrees with the generator's (its own
// comment: "the lattice itself is regenerated by consumers"). Kept here, not imported
// from the generator, because the generator is a build tool the engine never loads.
...
// isHexInBounds(q, r) -> is this axial coordinate a real hex INSIDE the galaxy lattice?
// The membership test is the generator's own (tools/generate_seed.js): a hex is in the
// galaxy iff its world-space centre lies within `galaxyParams.radius`. Both numbers come
// from the SEED (`galaxyParams.{radius, hexSize}`), so this invents nothing — it is the
// same rule that produced the seed's `stats.totalHexesInGalaxy`, and every seeded system
// and outpost passes it by construction.
//
// A hex coordinate is an INTEGER lattice point: a non-integer or non-finite q/r is not a
// hex at all and answers false, so the vehicle-location invariant rejects it as corruption
// rather than resolving a fractional "coordinate". A seed with no `galaxyParams` (a bare
// test fixture) has no lattice to bound, so this answers false there too — a bare-hex
// location is only meaningful on a generated galaxy.
function isHexInBounds(q, r) {
  const gp = index().galaxyParams;
  if (!gp || typeof gp.radius !== 'number' || typeof gp.hexSize !== 'number') return false;
  if (!Number.isInteger(q) || !Number.isInteger(r)) return false;
  const p = hexToPixel(q, r, gp.hexSize);
