...
Stage 2 has started. The decision checklist is answered (19-07-26, `docs/phase-1-tuning.md`) and the **invariant harness has landed** — `invariants.js` (invariants 1/2/3 + the §15.2 integer convention, asserted every tick) and `serialize.js` (canonical stringify + hashing for the determinism check), both under `tests/`. **`state.js` has landed** — entity constructors (Guild, Venture, Reserve, Syndicate) plus `createState(scenario)`, the one-time tick-0 bootstrap; it holds no game-balance numbers of its own. **`tick.js` has now landed too, as a scaffold** — `tick(state, actions)` is pure and runs the eight §15.6 steps in their fixed order, but every step is currently a documented no-op seam (production, pricing, etc. all need `economy.js`/`territory.js`/`bots.js`, none of which exist yet). Still to come: real logic behind those seams, action intake (`actions.js`), and the walking skeleton's actual starting scenario (2 guilds, one venture each, per `docs/phase-1-tuning.md`) that will give the eight steps something to do (see `docs/roadmap.md`, Phase 1 Stage 2).
