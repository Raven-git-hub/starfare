...
Stage 2 has started. The decision checklist is answered (19-07-26, `docs/phase-1-tuning.md`) and the **invariant harness has landed** — `invariants.js` (invariants 1/2/3 + the §15.2 integer convention, asserted every tick) and `serialize.js` (canonical stringify + hashing for the determinism check), both under `tests/`. **`state.js` has now landed too** — entity constructors (Guild, Venture, Reserve, Syndicate) plus `createState(scenario)`, the one-time tick-0 bootstrap. It deliberately holds no game-balance numbers of its own; the walking skeleton's actual starting scenario (2 guilds, one venture each, per `docs/phase-1-tuning.md`) is still to come, as is the pure `tick(state, actions)` loop that will consume what `state.js` builds (see `docs/roadmap.md`, Phase 1 Stage 2).
