...
*(Status note, updated 01-08-26. Consolidated to ONE generator: `generate_seed.js` now implements pipeline steps 1–3, 5, 6, 7, 8 and 10 — systems, naming, planets **with archetypes and resource nodes**, outposts + Citadel, starter-eligible tagging, and validation (`validation: PASSED` on seed 7331). Archetypes/nodes are applied as a decorating pass AFTER the base draws, so the system/planet/outpost skeleton stays byte-identical to the bare seed and `data/test_claims.json` still lines up. The former reference checkpoint `generate_seed_recovered_partial.js` is **removed** — its logic is absorbed here, with **Terran corrected to §2** (15 nodes incl. gold/silver/tungsten; the recovered copy wrongly had 9 and dropped the Molten metals), which also retires the old 5,290-vs-5,089 planet-count discrepancy (one generator, 5,290). Node **richness/yield is deliberately not generated** — an undecided number kept out of the canonical seed rather than placeheld. Steps 4 and 9 — ring classification and the rare-tier repair pass — remain in **no** code; their descriptions below, and §2's ~41%/~3.6% figures, are the verification targets for the next slice.)*
...
- `tools/generate_seed.js` — the single generator, at pipeline steps 1–3, 5, 6, 7, 8, 10: systems, naming, planets **with archetypes and resource nodes**, outposts, the Citadel, starter-eligible tags, and validation. Writes a **~3.3MB** seed for a 101,287-hex galaxy (1,500 systems, 5,290 planets, 23,420 resource nodes, 9 outposts). Stores no hexes. Still missing steps 4 and 9 (ring classification, rare-tier repair) — the next slice. Determinism verified by running it: same seed → **byte-identical file**, different seed → different galaxy. The `generatedAt` field was dropped (ruling 20-07-26) — provenance is git's job — so the file is byte-identical across runs, not merely content-identical.
