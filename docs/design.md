...
**Gate 1 — the source waterfall (per good, per system).** Once every producing venture has deposited its output into the system pool, the pool **empties completely every tick** into three **forks**, poured in the player's chosen **priority order**, each fork taking up to the amount set for it until the pool is exhausted. Nothing is left sitting in the pool as a fourth residue — every unit produced this tick ends up in exactly one fork. The three forks:
...
2. **Downstream** — feeding the consuming ventures, up to **full-tilt demand** (Gate 2): the most those ventures could use at maximum capacity. The player may cap it *lower* (to hoard more); untouched, it feeds them fully.
