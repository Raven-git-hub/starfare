...
- **The player order book.** Guilds list asks and bids at any price, sorted conventionally; the book sits alongside the Syndicate line and can diverge from it freely in either direction. The gap between the steady Syndicate line and the scatter of player trades is the "trust nobody, need everybody" premise made visible. Trading with the Syndicate is never strictly better or worse than trading with a guild — reliable and unglamorous beside noisier and potentially more profitable. The book proper needs multiple actors, so it lands with Phase 3.
...
Supporting mechanics under consideration: **anti-hoarding safeguards** (decay/spoilage, storage costs scaling with stockpile, use-it-or-lose-it surplus expiry); **shortage consequences beyond price** (reduced fleet capacity, lost route access, reduced influence generation) so scarcity stays existential; and a **survival floor** below which a guild _degrades rather than dies_, so a successful fuel squeeze can't crash weaker guilds entirely out of the game.
...
2. **Production is force-sold.** "Produce" puts fuel straight into the market pool and pays the producer at the posted price; even the Refiner buys back its own 8/turn need. This is the "fuel cannot be sold privately" rule (Section 3) already embodied — the reserve is fuel's only counterparty.
...
*(Status note, 17-07: steps 4–6 and 8–10 below describe the unrecovered 15-07 build. The generator actually in the repo implements only steps 1–3 and 7, with bare placeholder planets. The descriptions stand as the re-implementation spec — roadmap Track G — and the check marks record that the design questions are answered, not that code exists.)*
...
**Referenced but missing from the repo** — each was built in a session whose artifact was never saved. Recover from the local machine if a copy survives; otherwise rebuild against its documented spec: `galaxy-map-hex.html` (the original map prototype and JavaScript textbook), `system_planet_ui_mockup.html` (the Index UI, fully specified below), `SyndicateMarketplace.jsx` (the Exchange mockup, specified in §5), the 15-07 `generate_seed.js` (Track G), and `galaxy-state-model.md` (superseded by §15 — referenced only by an old code comment; nothing to recover).
...
1. **Syndicate fuel-utility governance details** — exact voting mechanics, cap enforcement, and how cartel capture is detected and challenged.
