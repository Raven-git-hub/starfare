...
Within a system, a good produced by a guild's ventures flows **producer → (Syndicate / stockpile / onward production) → consuming ventures**, through three **gates**. Resources are system-scoped (ruling B1, §15.2), so the whole flow is **per good, per system**. Production is automated: the player sets a standing policy and the tick executes it every advance. (This is the design for the interactive pipeline slices; slice 1 — the *read-only* Production view that surfaces the current picture — is landed, roadmap Phase 1 Stage 2.)

**Gate 1 — the source fork (per good, per system).** Once every producing venture has deposited its output into the system pool, the good is forked three ways:
1. **Syndicate commitment** — the sum of the producing ventures' fixed **output commitments** (the licence-fee lever above: a fixed quantity = commitment% × *standard* output, attributed per venture so a breach lands on the right one). This is **not a live dial** — it is set at licence time; the flow displays it.
2. **Stockpile** — a player-set share held in the system pool.
3. **Production** — the remainder, sent onward to consuming ventures, **capped at downstream demand** (Gate 2). Anything above that cap spills to stockpile automatically — the production dial only goes up to what onward production needs.

So the player's live control at Gate 1 is the **stockpile-vs-production split of the post-commitment remainder**.

**Gate 2 — downstream demand (the maths).** How much of the good the consuming ventures actually need. **Naive now:** Σ over consuming ventures of (that venture's batch-target × the good's input-qty-per-batch). **Effective later** `[FUTURE]`: capping each consumer's demand by its *other* scarce inputs (a `titanium_alloy` line also needs carbon), which couples every good's demand to every other good's supply — a truer figure deferred so it can be swapped in without a schema break. The UI shows produced-vs-demand and whether it is short. The **engine computes per-TICK; the UI displays per-hour** — a display transform that needs a tick-duration constant which is **not yet ruled** (open item, `docs/phase-1-tuning.md`), so per-hour is a clean seam until then.

**Gate 3 — under-supply distribution.** If the player sends **less** than downstream demand to production, the shortfall is distributed across the consuming ventures by a player-set policy — **by percentage or by quantity** per consumer. There is **no separate priority-rank field**: the allocation numbers *are* the priority. Where the player has set nothing, the engine falls back to the **default — first-come-first-served in venture-establishment order** (today's behaviour, now a documented, deliberate fallback rather than the only rule). A **deterministic tie-break** is required (invariant 9).

**One number, three jobs.** A venture's output-commitment quantity (Licence Fee, above) is simultaneously the Gate-1 Syndicate cut, the base its **dividends** are computed on (Leasing, below), and — if missed (e.g. a downstream refinery the player starves via Gate 3 produces less than it owes) — the **breach** that costs the full basic fee. Starving your own licensed line therefore carries real downside: the rules-based tension the flow exists to create. (Nothing sells yet — the Syndicate Exchange price step is a stub — so until the licence slice the Syndicate arm is a *displayed reservation*, backed by the reserved `Venture.syndicateCommitment` placeholder, `0`/unlicensed for every venture today.)

**The System Production Profile — where the settings live.** The gates are player-set parameters the tick must read, so they are **owned state**: a per-guild, **per-system** standing policy (one entry per good) that `stepProduction` consults every advance — the thing the galaxy-state advancement reads to know how to split each good. Two principles are **settled**:
- **Advisory, with the default as fallback.** The profile never drives the engine into an illegal state. Anything it does not cover — a good with no entry, a consumer it doesn't name — falls back to the FCFS default. A **stale** entry (a venture the profile names but that has since been removed) is ignored; a **new** venture the profile is silent about defaults. The engine can never break because the profile is out of date; it just flags "you have unallocated titanium, go set it."
- **Self-healing / operator reminder.** When settings would over-commit a resource the guild does not have, or when ventures are added or removed, the profile must **re-normalise** and the UI must remind the player to review — "the game reminds them," from the design vision.

**OPEN — to settle in design before the interactive build slice:** the profile's exact **field shape** (how a good's Gate-1 split and Gate-3 distribution are stored), and the precise **self-heal / re-normalisation mechanism** (what "re-adjust when over-committed" does, deterministically). These are the hinge the interactive slice hangs on — settle them, record them here, then prompt the build.
