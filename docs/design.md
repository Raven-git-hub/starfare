...
- **Administering the fuel utility** — it controls the flow of Deuterium, the only viable fuel in the galaxy, from reserve to allocation (Sections 3 and 8), and is its **sole legal refiner**: the refinement tech is proprietary and an open secret, monopolised not because no guild could copy it but to keep fuel access fair and stop any one guild cornering it — a founding reason the Syndicate exists.
...
*(Status: **implemented (Track G, 02-08-26); see §16 and §20.** Archetypes, resource nodes, the rare-tier ring gradient, the repair pass, and validation are in `tools/generate_seed.js` and materialised in `data/seed.json` — every planet carries `archetype` + `resourceNodes` — with §16 giving the pipeline and §20 the verification on seed 7331. One gap remains: `Planet.stats` — atmosphere, size, climate, explicit biomes — is still unbuilt; archetype carries the whole economic load, and it's enough for the economy (open question #33).)*
...
The galaxy is split into three concentric rings by distance from the Citadel (0–⅓ radius = inner, ⅓–⅔ = middle, ⅔–1 = outer), and the three **rare-tier** archetypes — Molten, Irradiated, Crystalline — get a per-ring weight multiplier on top of their base weight: **inner ×4, middle ×1, outer ×0.25**. Measured on a real seed in the unrecovered 15-07 build, that produced ~41% rare-tier planets in the inner ring against ~3.6% in the outer — an ~11× spread. The Track G re-implementation (02-08-26) measures ~38.7% inner / ~14.4% middle / ~4.3% outer — a 9.0× spread — on seed 7331: close to the 15-07 targets and exactly what the sourced ×4 / ×1 / ×0.25 multipliers predict (inner rare share = 56/142 ≈ 39%). The residual gap from 41%/3.6% is this generator's own inner-ring spatial layout, not a different rule — the multipliers are the contract and were implemented unchanged.
...
## 4. Advanced Manufacturing: Assets, Droids, and Ships
...
Assets fall into two functional categories governed by different existing systems. **Consumable/deployable assets** (transport ships, weapons, security droids) get used, committed, or destroyed, and route through the market. **Infrastructure assets** (station modules, factory/refinery modules, installed droid capacity) are persistent modifiers on a site or territory, and route through the venture/territory system.
...
**Venture ownership and territory ownership are now independent.** A guild can lease space inside another guild's territory and bring its own factory and droid workforce. This creates the territory owner's third economic lever (land rent) and a new tenant vulnerability: lease-based pressure (denied renewal, hostile rent hikes), analogous to how routes are exposed to blockades. Both are **now resolved** in Section 5's lease treatment (#23): lease **access is contractual, not a legality matter** — an active lease confers a standing right of access to the venture's site, so a tenant's presence is never trespass — and placed **assets are relocatable** — on termination the machine recovers via orbital limbo and the tenant hauls it home. The consequence is deliberate: the landlord's leverage is **weaker** than a fixed-asset reading assumed, because the tenant can always leave with their machinery.

### Space vs. Planet

A clean working rule was set: **space = claim/reach, planet = production.** Outposts are deployable in space purely to extend territory, with no production function; factory and refinery modules are planet-bound and are what droid complements staff. The outpost's "fuller purpose deferred" note is now partly answered — an outpost is the **mid-chain anchor that extends a guild's Toll Path reach from ~10 to ~20 hexes** (Section 2).

### Ventures and Ground Assets *(04-08-26)*

A venture and the machine it runs are **two separate things** — a split the lease mechanics (Section 5) rest on. An **asset** is the physical machine; a **venture** is the operating company around it (licence, droid workforce, shareholders, automation). So **an asset cannot run itself** — it needs a venture's administration and workforce to operate — and **a venture can outlive its asset** — if the machine breaks down, the venture persists until it is repaired or replaced. A venture references its machine by a nullable `assetId` (§15.4); assets are **guild-owned**, and their fuller lifecycle — condition, decay, relocatability — is Section 5's lease/limbo treatment.

There are exactly **two kinds of ground-deployable asset** (ships and droids are separate — mobile/consumable, not ground production):

- **Miner** — extracts one raw resource (Tier 1). Deploys **only on a resource node**; produces that node's `resourceType`.
- **Factory** — deploys **only on a settlement slot**, and its operator **configures** it to one of three modes: **refine** (Tier 1→2), **manufacture** (Tier 2→3), or **construct** (Tier 3→4). One machine with a dial, not three machines.

So the four-tier tree (§15.2: Raw → Processed → Parts → Constructed Assets) is covered by just these two: the miner makes Tier 1, the factory makes Tiers 2–4 by mode. This is already the code's shape in miniature — a mining venture (`resourceType`, on a node) *is* the miner; a refining venture (`recipeId`, on a slot) *is* a factory in refine mode. Manufacture and construct are higher-tier factory recipes **not yet built** (`sim/actions.js` currently validates only `mining`/`refining`); a factory's mode follows from the recipe it is assigned, so it stays a single source of truth rather than a second stored flag.

### Droids

A single droid type (no quality tiers for v1) deploys into one of two roles, reassignable with a time delay:

- **Factory role** staffs a venture's production. Ventures have a min/max droid complement; below minimum, the venture doesn't operate. Output scales from min to max on an **exponential curve** — concentration is heavily rewarded, with a hard cap at max.
...
Droids decay only while deployed (never idle in storage), with decay slowed by a flat-credit **maintenance spend** — keeping it in the same mechanical bucket as other venture dials. Combat losses are a separate, rarer layer on top of routine decay. It is yet to be decided if droid states are tracked individually, or by pool. Idle stockpiles incur storage costs scaling with size, consistent with the fuel model, with the storyteller as a rare dramatic backstop against extreme hoarding (factory recalls, droids gaining sentience). Complement size and condition are **hidden by default**, inferable only through the future espionage system.
...
### The Production Flow — Gates & the System Production Profile *(05-08-26)*

Within a system, a good produced by a guild's ventures flows **producer → the system pool → three player-ranked forks → (Syndicate / stockpile / consuming ventures)**, through three **gates**. Resources are system-scoped (ruling B1, §15.2), so the whole flow is **per good, per system**. Production is automated: the player sets a standing policy and the tick executes it every advance. (This is the design for the interactive pipeline slices; slice 1 — the *read-only* Production view that surfaces the current picture — is landed, roadmap Phase 1 Stage 2.)

**Gate 1 — the source waterfall (per good, per system).** Each tick, once every producing venture has deposited its fresh output, **this tick's production** is routed completely into three **forks**, poured in the player's chosen **priority order**, each fork taking up to the amount set for it until that fresh output is exhausted. Every unit produced this tick ends up in exactly one fork — nothing fresh is left unrouted. **The forks route this tick's flow only; the stockpile accumulated on prior ticks sits untouched** — it is not re-poured, and Downstream never draws it down (see below). The three forks:

1. **Syndicate** — the venture's licence commitment. The flow **displays the licence minimum**, but that minimum is a **ceiling on what auto-sends, not a floor**: the player may dial this fork *down*, even to zero, keeping the resource and eating the breach fee for the shortfall (a real, costly choice — "if you want the resource that badly, negate the commitment and shoulder the fee"). The player may never send *more* than the minimum. Inert at **0** for every venture today (no licences yet).
2. **Downstream** — feeding the consuming ventures from this tick's flow, up to **full-tilt demand** (Gate 2): the most those ventures could use at maximum capacity. The player may cap it *lower* (to hoard more); untouched, it feeds them fully. A venture the fresh flow can't fully feed **stalls for the tick — it does not draw down the accumulated stockpile** (a stockpile-drawdown / buffer mechanism is deferred to a later slice).
3. **Stockpile** — held in the system pool. The player sets this as a **percentage of this tick's production or a flat quantity** (the same control expresses both; the flow shows the discrete number either way). Its behaviour depends on **rank**: ranked high (1st or 2nd) the number is a **floor** — that much is set aside *before* lower forks are fed; ranked last it is a **catch-all**, its number moot, absorbing everything the higher forks didn't take.

**Priority genuinely wins, even when it starves what's below.** Each fork takes what it asks for, in the player's order, and lower forks make do with the remainder; a high-ranked stockpile floor *will* starve downstream, and that is the intended lever, not a bug to "fix." No fork can push the pool negative — it only ever pours from what remains — so the waterfall **cannot over-commit itself** (this is what lets Gate 2's demand be a fixed full-capacity figure; whatever downstream doesn't take lands safely in stockpile). The player may rank the three forks **any way they wish and owns the consequences** — the Syndicate need not be first. The **default order is Syndicate → Downstream → Stockpile** (pay the licence, feed the factories, pile up the rest), which reproduces today's behaviour when the player sets nothing.

**Gate 2 — downstream demand (the maths).** How much of the good the consuming ventures need **at full capacity** — the figure the Downstream fork is capped at. **Naive now:** Σ over consuming ventures of (that venture's batch-target × the good's input-qty-per-batch), where batch-target is the venture's *rated* throughput. Crucially this is a **full-tilt figure that does not shrink when a consumer runs slow** — a refinery throttled by a *different* scarce input still counts its full titanium demand here — which is exactly why the Downstream fork's cap is a stable number the player can reason about. **Effective later** `[FUTURE]`: capping each consumer's demand by its *other* scarce inputs (a `titanium_alloy` line also needs carbon), which couples every good's demand to every other good's supply — a truer figure deferred so it can be swapped in without a schema break. The **engine computes per-TICK; the UI displays per-hour** — a display transform that needs a tick-duration constant which is **not yet ruled** (open item, `docs/phase-1-tuning.md`), so per-hour is a clean seam until then.

**Gate 3 — under-supply distribution across consumers.** When the Downstream fork can't fully feed the **several consuming ventures** competing for a good — because a higher-priority Gate-1 fork took the resource first, the player capped downstream below full, or production simply fell short — Gate 3 rations what arrived. The per-venture control is a **throttle: a percentage of that venture's own full-tilt demand** ("run this line at 75%") — *not* a share of the total (which moves tick to tick and so promises nothing stable), and *not* a rank. When the throttled asks collectively exceed what arrived, **every line scales down together, in proportion to its throttled ask** — a deterministic split that singles no one out. Goods are integers, so a split rarely divides evenly; the rounding remainder goes to the **earliest-established line** (the same establishment-order tie-break the default uses, invariant 9), keeping the division exact to the unit and never negative. When the throttles instead ask for *less* than arrived, the surplus is **not stranded**: it waterfalls on down the tree through the next tier's Gate 1 into stockpile (the same overflow property Gate 1 has). **Priority is emergent, not a separate field:** a bigger throttle weight simply *is* "this line matters more," so a line at 100% keeps its share as the pool shrinks while a 25% line starves first — and the *hard* "the licence is fed before a single unit goes elsewhere" guarantee is not Gate 3's job at all, it already lives one gate up in the Syndicate fork (Gate 1 above). Gate 3 only ever divides what's left among the guild's own lines. Where the player has throttled nothing, the engine falls back to the **default — first-come-first-served in venture-establishment order** (today's behaviour, now a documented, deliberate fallback).

**Breach comes from two places.** A venture's output-commitment quantity (Licence Fee, above) is simultaneously the Gate-1 Syndicate fork's minimum, the base its **dividends** are computed on (Leasing, below), and — if the delivered commitment falls short — the **breach** that costs the full basic fee. It can fall short two ways: the player **dials the Syndicate fork below its minimum** at Gate 1 (a deliberate choice), or a downstream refinery the player **starves via Gate 3** produces less than *its own* committed output. Either way the shortfall is a breach: starving your commitment carries real downside — the rules-based tension the flow exists to create. (Nothing sells yet — the Syndicate Exchange price step is a stub — so until the licence slice the Syndicate arm is a *displayed reservation*, backed by the reserved `Venture.syndicateCommitment` placeholder, `0`/unlicensed for every venture today.)

**The System Production Profile — where the settings live.** The gates are player-set parameters the tick must read, so they are **owned state**: a per-guild, **per-system** standing policy (one entry per good) that `stepProduction` consults every advance. For Gate 1 an entry stores, per good, the **fork ordering** and **each fork's amount** — the stockpile amount expressed as a percentage or a flat quantity at the player's choice, the downstream amount capped at full-tilt demand, the Syndicate amount capped at the licence minimum. For Gate 3 an entry stores, keyed **per consuming venture** (not per good — a line burns its inputs in lockstep and so runs at one rate, not a rate per ingredient), an optional **throttle**: that venture → its percentage of its own full-tilt max. Ventures the profile does not name run at the default. The engine also **displays**, per consuming venture per good, what it needs (full-tilt max), what it is getting this tick (the discrete allocation), and that allocation as a percent both of its own max and of the total upstream input — because a Gate-1 downstream cap and a Gate-3 throttle **compound**, so the set percentage rarely equals the venture's actual rate and the real numbers must be on screen. Two principles are **settled**:
- **Advisory, with the default as fallback.** The profile never drives the engine into an illegal state. A good with **no entry** falls back to the default order with no stockpile floor and downstream fed fully — i.e. today's behaviour. Anything the profile does not cover — a consumer it doesn't name at Gate 3 — falls back to the FCFS default; a **stale** entry (a venture the profile names but that has since been removed) is ignored; a **new** venture the profile is silent about defaults. The engine can never break because the profile is out of date; it just **flags** "you have unallocated titanium, go set it" — it does not silently rewrite the player's stored intent.
- **Self-healing by construction (Gate 1) / reconciled at read time (Gate 3).** Gate 1's waterfall cannot over-commit: it is recomputed every tick from the current forks and the current demand, so a venture added or removed simply changes the demand figure the Downstream fork pours into — there is nothing stored to go stale. Gate 3's per-venture throttles *are* stored, so they reconcile at read time each tick: a throttle naming a **removed** venture is ignored, a **new** venture the profile is silent about falls to the default, the survivors re-split, and the player gets the review flag — the engine never rewrites the stored throttles itself. Because the split is a proportional scale-down of *whatever throttles currently resolve to real ventures*, it can never over-commit or drive a stockpile negative (invariant 3).

**SETTLED — both gates are designed; the interactive slice can be prompted.** Gate 1 (the source waterfall) and Gate 3 (the per-venture throttle with proportional scale-down) are both fixed above, and the System Production Profile is their owned home. What remains is **deferred, not open** — not blockers for this slice: the per-hour display constant (`docs/phase-1-tuning.md`), Gate 2's *effective* (cross-input) demand `[FUTURE]`, and the real Syndicate licences that make the commitment fork non-zero (a later slice). The build proceeds on naive demand, per-tick figures, and `syndicateCommitment` = 0.

### Reputation and Hostile Takeovers

Ventures carry a **fully visible reputation score** serving double duty as an investment-risk signal and a **takeover trigger**: hostile takeover eligibility unlocks only when reputation drops below a threshold, protecting healthy ventures from casual predation, and **only shareholders vote** on a takeover, keeping it a private-capital matter rather than galaxy politics. Rivals can deliberately suppress a target's reputation — primarily through **legitimate market pressure** (outbidding suppliers, poaching shipping partners via better toll rates, trade refusal), with **actual sabotage** as an optional higher-risk layer carrying exposure risk and legality consequences. Because reputation is visible, targets can see pressure building and respond: shore up suppliers, contest routes, appeal to council, dilute ownership toward allies, or arrange poison-pill side-deals. A distinct **guild-level reputation** could track vulture-tactic patterns, making predatory guilds harder trading partners over time. What minority investors receive in a takeover (likely a payout at takeover price) still needs defining — now folded, together with the lease-dissolution payout below, into open question #59.

### Leasing: Land Owner & Venture Owner *(05-08-26)*

Venture ownership is decoupled from land (§4), so a guild can lease a site inside another guild's territory and operate its own asset there. That splits the venture's controlling block between two parties with opposed interests: the **Land Owner (LO)**, who provides the site, and the **Venture Owner (VO)**, who brings and runs the machine.

**Ownership split.** On land a guild owns outright, the owner holds a **minimum 51%**. On leased land that 51% is split: the **VO holds exactly 25%, permanently** — it can neither buy more nor be diluted — and the **LO holds a minimum 26%**, and may buy up to **75%**. The remaining **0–49% float** is open to outside investors. Crucially, **control is the VO's**, held by contract and divorced from equity: `Venture.ownerGuildId` denotes *control*, which in a lease is a minority (25%) position (§15.4).

**Leased ventures are structurally takeover-proof.** The 51% is locked between two parties, neither dilutable below their floor, so an outsider can accumulate at most the 49% float — never a majority. **A hostile takeover of a leased venture is arithmetically impossible without the LO's cooperation.** This is deliberate: it is the compensation that makes the VO's otherwise poor economics signable — you earn little, but nobody can take it from you — and it recasts an LO-sanctioned takeover as *eviction with extra steps*.

**The economic trade, stated as a design position** (not a bug for a player to discover):

| Party | Sacrifices | Gains |
|---|---|---|
| **LO** | Resources and production capacity | Lease fee + dividends, option to buy up to 75%. **Sees no goods at all — cash only.** |
| **VO** | Credits — likely loses money on the venture | The right to produce and keep output, minus the committed minimum. **The goods are the point.** |
| **Syndicate** | The licence fee (**waived for leases**) | Output from an otherwise idle node. It wants productivity. |

The only reasons to be a VO: you have no land, you need production space, or you need a specific resource. A legitimate capital-vs-labour arrangement.

**The commitment percentage is the central negotiated term.** Lease licences are **free**, with a **minimum 25% of output committed** to the Syndicate. Why would a lease commit more than that floor? Because dividends flow only from committed output (below), and the LO's entire return is cash — so the LO wants commitment **high**, while the VO wants it **low** (everything committed is output they don't keep). Higher commitment also means a more ambitious licence target → higher reputation → higher share value → harder to take hostilely. That tension is the deal itself: the contract is the haggle over the commitment %.

**Dividends are locked to committed output.** *Shareholders are paid only on committed output; withheld output accrues to the controlling guild — the VO — alone.* This must be explicit: implement pro-rata splits on withheld output and the entire LO/VO trade inverts. In current terms, the withheld portion simply stays in the owner guild's (the VO's) `stockpiles`; the committed portion is delivered and its proceeds are the base dividends are computed on.

**VO transport gets no special case.** Instantaneous/backgrounded movement is rejected — every "goods move free" rule is a tax cut on the fuel market §8 depends on. A hex-range threshold is rejected too — it is a cliff that makes land value discontinuous and breaks when territory moves. Instead: intra-system movement is already free where the VO has presence (ruling B1), and cross-system the VO ships like anyone else, so the range limit is **emergent** (shipping eats the margin unless you have earned a foothold), with no parameter and no second transport regime. The goods live in the owner guild's `stockpiles` (the VO's — the per-venture `outputStockpile` was retired 02-08-26), and the venture's **site is the shipment origin** — explicitly *not* an outpost-like claim, since letting a VO plant a space claim on LO land is the territorial grab the lease exists to avoid. Access is a **standing right derived from the active lease** (never a stored flag): the lease converts what would be trespass (§2) into a contractual right, so the tenant's presence is never a §7 matter, and once a transport clears the site ordinary rules apply — the LO holds no leverage over the exit. (Consequence: the LO may be paid twice or three times — lease fee, dividends, and §2's extraction/processing tariff — which against a 25%-capped, credit-losing tenant may be one stream too many → #58.)

**Takeover — i.e. the lease ending — and where the asset goes.** Because a leased venture is takeover-proof, the only way it changes hands is the lease terminating, at which point **the VO packs up and leaves.** Assets are relocatable (this resolves #23): on termination the asset **packs itself into orbital limbo** — placed in orbit around the planet it occupied, awaiting collection by its owning guild. It cannot be touched, claimed, or intercepted by anyone else; it is an explicit, stated exception to interception. This frees the LO's slot immediately (no squatting, no wind-down, no dispute), needs nothing new in the data model (limbo is an asset `status`; the asset stores its own coordinates like any moveable asset, §15.3), and still **costs a haul** — collection is an ordinary system-to-system transport job with fuel and time against it, since the lease *was* the VO's presence in that system. The asset is never lost, but getting it back is never free. (The state likely generalises beyond leases — abandonment, dissolution, salvage-in-waiting.)

The ownership rule this forces was already planted in §15.4 and is worth stating plainly:

> **Assets are owned by guilds. Ventures merely operate them.**

So an LO holding 74% of a venture has **no claim on the VO's machinery** — the asset goes to limbo and the VO collects it. A venture's book value is therefore **pure goodwill** — reputation, licence terms, position, contracts, nothing physical — which is exactly what §5 says takeovers are *for*. On dissolution, outside investors receive **the value of their shares** and nothing else; how that value is computed and who funds it is #59.

**The dissolution payout, and tanking as a feature.** When a lease ends, the LO and VO **jointly fund the shareholder payout, split 51% LO / 49% VO** — the true proportion of the locked block (26/51 ≈ 51%, 25/51 ≈ 49%). This makes eviction cost something, deterring casual scalping, with the LO carrying the majority. An escape route is left open on purpose: the LO can buy out the float first (at the 75% ceiling the float is zero and eviction is free) — but that is expensive, slow, and *visible to the VO while it happens*: pressure, not prohibition. And **running the venture into the ground to cheapen the exit is a FEATURE, not a bug** — payout value is reputation-linked, so tanking cuts the liability. Trailing-average valuation was proposed as a fix and rejected; the chaos is wanted, and it is defensible because reputation is fully visible (§5) so it is a telegraphed betrayal investors can watch and dump into; because the weapon is asymmetric (tanking needs operational control, which is the VO's, so **the LO holds the eviction trigger and the VO holds the tanking weapon** — the cash-poor tenant's one piece of leverage, and the one that costs the landlord most); and because the counterweight already exists (§5's guild-level reputation tracking vulture patterns — do it twice and nobody leases to or invests with you again). The stable equilibrium is *collusion* — both insiders running it down together and stiffing the float — thoroughly on-theme. Two collisions logged: deliberate tanking is exactly a storyteller rapid-collapse signal (→ #62), and whether the payout fires on non-renewal as well as eviction needs a ruling (→ #60).

**Syndicate forced closure — the brake for leased ventures.** For a normal owned venture, a reputation slide makes it takeover-eligible (§5), so rivals may seize it before its owner collects a cheap exit — a working deterrent from existing parts. But a leased venture is takeover-proof, so that brake is missing exactly where tanking pays best. Forced closure fills the hole: **when a leased venture's reputation falls far enough, the Syndicate withdraws the licence, shuts it down, and pays the investors out.** The thresholds are ordered so the **takeover threshold sits above the closure threshold** — owned ventures get eaten by rivals on the way down, and only leased ones survive far enough to reach the Syndicate's cliff. The trap: payout value is reputation-linked, so closure fires when value is lowest — a naïve build would *reward* tanking and punish only honest failure. The correction reuses existing machinery: closure carries the **breach penalty already in the licence terms** (missed committed output = the full basic fee, §5) plus a **guild-level reputation mark** (§5), optionally a licence cooling-off for that venture type — so the tanker pays in standing and fines, not in payout size, preserving the ruling that tanking cheapens the exit. Funding holds conservation: the VO is cash-poor by design, and the **Syndicate ledger is exempt and may go negative** (roadmap invariant-3 rulings, §15.6), so the Syndicate fronts the payout and holds the guild in debt — an existing decision, no new mechanism. Crash-prone leases do not kill the float, they *price* it: high-variance shares trade at a discount with fatter upside for whoever reads the risk right, and since reputation is visible, pricing it is a **skill** — the "room for market intuition" §5 asks for. (Reputation is now load-bearing for five mechanics — takeover eligibility, forced closure, share value, payout size, licence standing — while the event set that moves it is still unspecified → #61.)
...
**Tier‑4 assets trade on a fundamentally different model**, because the Syndicate stocks none of them — ships, droids, deployable mining rigs, factory complexes, Outpost and Toll Gate construction kits:
...
- Salvage deliberately plugs into the **not-yet-designed condition/maintenance system** (Section 4's droid decay is its nearest relative): once that exists, condition feeds salvage directly, giving maintenance spend a second payoff beyond avoiding breakdown.
...
- **Scope is deliberately narrow.** The Syndicate moves *only goods being sold to it*. It will not ferry a settlement, an outpost, a droid complement, or any asset a guild intends to deploy elsewhere — otherwise it silently dissolves the vehicle-gated expansion goal that anchors onboarding (Section 13).
...
- **Fuel enters the pool through the Syndicate's refining monopoly, not guild refineries.** The Syndicate is the galaxy's **sole legal refiner** of Deuterium (proprietary tech, an open secret; the monopoly exists to stop any guild cornering fuel — §1). Guilds **mine raw Deuterium and sell it to the Syndicate** for credits + standing (the old "producers are paid in credits, not fuel" reward, moved upstream to the miners); the Syndicate refines it into pool fuel, so the pool can hold no more than the Deuterium mined — the **no-infinite-backend** property, preserved by construction. Guilds get fuel only by **buying from the pool** with their allocation, and may **legally hoard what they bought** — self-limited, since allocation is sized to real burn so any excess costs full market price (no spoilage/storage/expiry mechanic needed on top — do not re-add one). **Refining your own is always illegal and can never be licensed**: a guild may do it off the books into black-market fuel (pressure, not prohibition) at the risk of a **steep fine** (§7). This is the reframed Stockpiler — a legal buy-and-hold hoarder of *purchased* fuel, or an outright criminal running a black-market reserve. It **supersedes** the earlier conditional-refinery-licence / pledge-for-discount / volume-scaling model, now retired; full treatment in `docs/fuel-allocation-model.md` §6.

**Force-sell is moot under the monopoly.** With no legal guild fuel *production*, there is nothing to force-sell: the Phase-1 default is simply that **guilds don't refine fuel**, and legal hoarding exists only via *purchased* fuel (above). Illegal refining is possible in fiction but unenforced until the §7 fine system lands (Phase 5). Open question #48 is reframed accordingly — not "keep or drop force-sell," but "guilds don't produce fuel; the pool is Syndicate-refined from bought Deuterium."
...
- **OWNED** — numbers belonging to exactly one guild, changed only by that guild. Credits, hoards, ventures, droids, vehicles. *Easy: the galaxy total of anything owned is just the sum across guilds.*
...
- **Resources are system-scoped — ruling B1.** A guild's goods exist as a **separate pool per system it operates in** — the settled shape is `stockpiles: systemId → good→int`, not one flat guild pile. Within a system, goods are pooled and instantly usable by any of that guild's ventures there (across planets); between systems they do **not** commingle, so moving goods is an ordinary shipment (fuel + time, §6/§8) and a guild's *presence* in a system is what makes that system's pool usable. The guild-wide total across all its systems is a **pricing aggregate only** — it feeds Galactic Supply and the Syndicate's supply-derived value (§5, §15.4), never a pool anything draws from directly. This is what §5's leasing "the range limit is emergent" and "where the VO has presence" rest on: a Venture Owner's output lands in the VO's pool *in the leased system*, and reaching home is a cross-system haul like anyone else's. *(Agreed 05-08-26; re-keyed in code the same day via `sim/stock.js` — a **no-op slice** (one guild controls one system today, so the structure changed with byte-identical galactic-supply output). Every stockpile read/write now goes through `sim/stock.js`.)*
...
- **Live State** — the OWNED/SHARED/IN-FLIGHT layers. Claims, ventures, droids, credits, prices, shipments. Mutates every tick, forever, under real concurrency — needs a real datastore with atomic, crash-safe writes, not a hand-rewritten file.
...
The **derive, don't store** principle does most of the work here. Of ~101,000 hexes, only the ~1,500 systems and the scattered outposts/gates carry an explicit claim; the other ~99,500 hexes' ownership is **computed** from the nearest claim within its radius. The hex lattice itself is never stored at all — it's pure geometry, reproducible from `galaxyParams` (radius, hex size) plus the deterministic seed number. A moveable asset (transport, droid shipment) **stores its own location**; "what's on this hex" is a *lookup*, never data authored onto the hex — otherwise the same fact lives in two places and they drift.

### 15.4 Core Entities

**Guild** (OWNED): `id`, `name`, `isBot`, `credits` (≥0), `influence`, `fuelHoard` (≥0), `incomeRate` (the flat baseline allocation), `homeSystemId`/`homePlanetId` (the guild's starter home system and its Terran homeworld planet — reference only; the ownership source of truth is the guild's home claim, §15.2), `guildReputation`, `stockpiles` (map raw-good→int, ≥0 — the guild's holdings of each raw resource, **system-scoped per ruling B1 (§15.2)**: `systemId → (good→int)` (a separate pool per system the guild operates in), accessed only via `sim/stock.js`; **fuel is not here**, it lives in `fuelHoard` and is governed by the fuel-conservation law), `lifetimeProduced` (map good→int, **monotonic — only ever increases**; exists so starter-quest progress survives selling down stock, Section 13), `productionProfile` (the per-guild, per-system standing policy that routes each good through §5's three gates — shape and defaults in the **Production Profile** block below; accessed only via `sim/profile.js`, parallel to `sim/stock.js`). *(Snapshot telemetry, 05-08-26: `buildSnapshot` now emits **both** the flat guild-total `stockpiles` (Σ across systems, for existing readers) **and** the per-system breakdown `stockpilesBySystem` (`systemId → good→int`). The per-system view was the "add the field when a consumer needs it" case the roadmap anticipated — **landed** for the read-only Production view, which shows a system's own holdings. Both are derived views the galactic-supply consistency invariant already guards through `guildTotals`; neither is a second source of truth.)*

**Production Profile** (OWNED — the Guild's `productionProfile` field; §5's *System Production Profile*): the per-guild, per-system standing policy `stepProduction` reads to route each good through the three gates. It mirrors `stockpiles`' system-scoping (ruling B1) and is reached only through `sim/profile.js` (parallel to `sim/stock.js`), so the nested shape lives in exactly one file:

    productionProfile: { [systemId]: {
      goods: { [good]: {
        order:         ["syndicate", "downstream", "stockpile"],       // Gate-1 fork priority — a permutation of the three
        downstreamPct: <int 0..100>,                                   // Gate-1 downstream cap, % of full-tilt demand
        stockpile:     { mode: "percent" | "quantity", value: <int> }  // Gate-1 stockpile fork amount
      } },
      throttles: { [ventureId]: <int 0..100> }                         // Gate-3 per-line throttle, % of that venture's own full-tilt
    } } }

**Sparse: an absent key means its default, and a good — or a whole system — with no entry behaves exactly as today.** Defaults: `order` = `["syndicate","downstream","stockpile"]` (pay the licence, feed the factories, pile the rest); `downstreamPct` = `100` (feed downstream fully); `stockpile` = `{ percent, 0 }` (keep nothing beyond overflow); a venture absent from `throttles` runs the **FCFS-in-establishment-order default**. Gate 1 is keyed by **good** (routing a resource); Gate 3 by **consuming venture** (throttling a line) — a line burns its inputs in lockstep, so its throttle is one number, never per-ingredient. **The Syndicate fork's amount is not stored here**: it is the venture's licence minimum (the reserved `Venture.syndicateCommitment`, `0`/unlicensed today), read from there so the commitment keeps a single source (invariant 5) rather than a copy in the profile.

**All percentages are integers 0–100** — §15.2's integer discipline extended to ratios, so the gate arithmetic stays exact and deterministic (invariant 9). **The engine reads the profile every tick and never writes it** (§5): a stale entry — a `throttles` key naming a since-removed venture, or a `goods` key for a good no longer produced — is ignored at read time and the survivors reconcile; a new venture the profile is silent about takes the default; an over-set value cannot drive a pool negative, because Gate 1 pours only from what remains and Gate 3 scales down proportionally (invariant 3). The player is **flagged to review** when the profile stops cleanly covering reality, but the stored intent is left untouched until the player changes it. *(Landed as inert owned state (slice 2a-i): `createGuild` defaults it to `{}` (copied through `cloneProfile` so a caller can't alias into engine state), the `sim/profile.js` accessor owns the nested shape (parallel to `sim/stock.js`), and the `setProductionProfile` action stores a per-`(guild, system)` policy — validated (integer 0–100 ratios, `order` a permutation of the three forks, known stockpile goods) but **advisory**: a `throttles` key naming a not-yet-existing venture, or a `goods` key for a good with no producer, is deliberately accepted. **Reserved / not-yet-built:** `stepProduction`'s consumption of the profile is the one remaining piece (slice 2a-ii) — the tick reads none of it today, so storing a profile is a proven no-op on production output. Recorded here first, per the seam rule, so the build reads the shape rather than inventing it.)*

**Venture** (OWNED): `id`, `ownerGuildId`, `type`, `assetId` (nullable — the ground **Asset** this venture operates; **reserved, not-yet-built**, §4/§15.4), `licenceId`, `siteId` (the resource node or settlement slot it occupies — reference only), `systemId` (the system that site sits in — a **denormalised** copy of `getSite(siteId).systemId` stamped at establish, mirroring `resourceType`; it is the pool key production deposits into / draws from under ruling B1, and `invariants.js` guards it against the site (`venture-system-matches-site`) so the copy can't drift), `resourceType` (the raw good a mining venture extracts; its output flows to the owner guild's `stockpiles`), `recipeId` (for a **refining** venture (03-08-26): the `sim/recipes.js` recipe it runs — a **multi-input** batch (04-08-26): consuming EACH of the recipe's input goods from and producing its output good into the owner's `stockpiles`, up to `productionRate` batches/tick, throttled by the scarcest input; a venture sets `resourceType` XOR `recipeId`), `productionRate`, `syndicateCommitment` (**reserved placeholder (05-08-26)**: the share of this venture's output promised to the Syndicate under its future **Licence** — §5's output-commitment — stored as an engine-owned **integer per-tick committed quantity** so it sums cleanly in a view's totals; **0 = unlicensed for every venture today**, since no licence system exists yet and no path sets it otherwise. Defaulted in `createVenture` and carried through the establish path, so it is engine-owned — the read-only **Production view** reads it beside every producer/consumer and in the totals instead of inventing its own number. **NO tick step reads it**, so carrying it is a behavioural no-op; when slice 3's real licences land, the `Licence` (below) becomes its source), `inputStockpiles`, `droidComplement` (min/max), `reputation`, `shareholders`, `automation` (thresholds/targets/stop conditions). *(Ventures carry no fuel/energy draw: production is renewable-powered and consumes no Deuterium — §3. Fuel is burned by spacecraft, via `Vehicle.fuelCostToRun`.)* *(The typeless per-venture `outputStockpile` scalar was **retired 02-08-26**: produced goods are typed and go straight into the owner guild's `stockpiles` — one home, no double-counting — with producer attribution preserved via `lifetimeProduced`. Reversible if a produced-but-unsold-at-venture state is later needed.)* *(Goods are three kinds (`sim/resources.js`): **raw** (minable, in archetype pools), **processed** (refined into stockpiles — the Phase-2 catalog, 11 goods as of 04-08-26, e.g. `titanium_alloy`), and the special **fuel** (`deuterium_fuel`, held in reserve/hoards, governed by invariant 1). Raw + processed are the legal stockpile keys and the galactic-supply rows; the **recipe catalog** (`sim/recipes.js`) is the fixed set of raw->processed conversions a refining venture may run — each a **multi-input batch** — guarded like the resource vocabulary against referencing a good that doesn't exist.)* *(`ownerGuildId` denotes **control**, not majority equity: the guild whose decisions run the venture — in a lease the Venture Owner, on a locked 25% (§5). Equity lives in `shareholders`; the two can diverge.)*
...
**Droid pool** (OWNED): `ownerGuildId`, `role` (factory/security), `assignedTo`, `condition`, `maintenanceSpend`. *(Individual-vs-pool tracking still undecided, Section 4.)*
...
**Fuel Utility** (SHARED, the special market): `reserveLevel` (≥0), `currentPrice`/`priceBand`, `ownershipStakes` (capped ~20–25%), `interventions` (active council measures), `refiningContracts` (contracts to **operate the Syndicate's refining monopoly** (§8) — the Syndicate is the sole legal refiner; these are *not* guild private-refinery licences). Processed Deuterium Fuel is not privately tradable *or refinable*, so this *is* its market; the pool is filled by the Syndicate refining raw Deuterium bought from guild miners (§8).

**Territory Map** (SHARED): one row per **system, outpost, or gate** — never per hex — `claimId`, `ownerGuildId` (single source of truth), `claimedAtTick`, `claimRadius`, `securityLevel`, `contested`. *(Implemented 03-08-26: a Phase-1 claim references a seed landmark by `landmarkId`+`landmarkKind` (citadel/outpost/system); the **claim-integrity** invariant asserts each resolves to a real landmark of that kind, and the **guild-home** invariant ties a guild's denormalised `homeSystemId`/`homePlanetId` to a real starter system + its home claim — the territory analogue of the site-occupancy invariant.)*
...
- **`ventureId` is gone from the seed**, because it was never seed data: which venture occupies a node is a **live-state fact that changes during play**. Storing it in the static seed would violate the seed/live split *and* single-source-of-truth in one move. It belongs in a claims-style map (`resourceNodeId → ventureId`) once live state exists — exactly the pattern `generate_test_claims.js` already uses for territory. (Note this is the same fact as Venture's `siteId` above, seen from the other end; `siteId` is the authoritative one.) *(Implemented 02-08-26: the venture stores `siteId` — a resource node **or** a settlement slot; occupancy is the derived lookup `sim/occupancy.js`, rebuilt from the ventures, never written onto the site. `sim/seed.js` resolves a `siteId` against the static seed, and `sim/invariants.js` asserts every `siteId` resolves to a real site, at most one venture occupies any site, and a mining venture's `resourceType` matches its node's — the guard that lets `resourceType` safely stay a denormalised copy.)*
...
4. **Atomicity of shared-state mutation** — any read-modify-write on SHARED state (buying from the reserve, claiming a resource node, casting a vote) must be atomic. Two actors must never interleave on the same shared number. This is the entire class of bug behind every money-printer and item-dupe exploit in every game economy ever shipped, and it's the reason multiplayer is deferred rather than attempted early. *No dedicated tripwire exists in Phase 1 — single-process intake drains one action window sequentially, so there is no concurrent interleaving to catch; the first-valid-wins discipline (§15.6; `sim/actions.js`, `sim/run.js`) is the standing substitute and **is** tested. Invariant 4's own mechanical tripwire is the **Phase 3 adversarial ~200-actor squeeze** against a single shared row (§15.7; roadmap Phase 3), once Postgres transactions make real concurrency possible.*
...
- **Live-recomputed.** Anything touching SHARED state cannot be scheduled ahead, because its outcome depends on what *other* actors do between now and then. A shipment's *route and burn* are knowable in advance; what that burn actually *costs* depends on the fuel pool everyone is drawing from, which nobody has decided yet. Pricing, fuel allocation, and any consumption of a SHARED resource must be recomputed fresh, every tick. (Note a purely local venture is the opposite case — it touches no SHARED state, since production is renewable-powered and draws no fuel, §3 — so it belongs to the pure-schedule bucket above, not here.)
...
**The driver (Phase 1): `advance(state, actions)`.** The loop above is realised in `sim/run.js` as one composed step — *intake → tick → assert*: apply this window's moves (validate-as-they-arrive, first-valid-wins), advance the eight steps, then run the invariant tripwires and halt loudly on any violation — so invariants 1/2/3 are asserted *every tick* by the running loop, not only by tests. Two Phase-1 realisations to record. **Action timing:** the driver drains the whole action window in one pass per tick, whereas §15.7's server drains continuously as requests arrive; the *discipline* is identical (one shared state, each move validated against state-as-it-stands, first-valid-wins) and only the drain timing differs — this is explicitly *not* the rejected batch-private-copies model. Intake runs *before* the tick's own steps, so a window's moves execute against the price the previous tick posted (§8 semantic #1). **The boot lifecycle:** a fresh server assembles a *guild-less zero-state* (`sim/scenarios/zero-state.js` — reserve, Syndicate + ledger, a world that merely references the seed, the Citadel and the seed's 9 outposts as Syndicate waystation claims (each pointing at a real seed landmark), **no guilds**) and simply waits; it begins ticking when the owner founds the first guild. Founding is itself an action (`foundGuild`): the new guild's starting credits are **debited from the Syndicate ledger**, so they *move* rather than appear — invariant 2 holds through founding exactly as through any trade, and "a guild exists" becomes "a guild was founded, and here is where its money came from." *(A second play action, `establishVenture` (03-08-26), seats a new mining venture on a real, unoccupied seed resource node — occupancy-guarded up front and backstopped by the site-occupancy invariant — so a galaxy can be built up interactively and then advanced; it moves no credits/fuel and begins producing on the next tick.)* **The dev harness (03-08-26):** `sim/server.js` wraps this same driver in a thin HTTP server for the interactive testbed — one in-memory live state, `POST /action` intakes a move (no tick), `POST /tick` advances one turn, `GET /snapshot` reads the lens, `POST /reset` returns to the zero-state. State lives behind a single get/set holder (the seam Phase 2 swaps for Postgres) and the server carries no game logic — every mutation goes through `intake`/`advance`, so the tests and invariants still guard the one engine. It is deliberately **not** the §15.7 persistent server: ephemeral, single-user, manual-tick, shipped in a container (port 7331) to rehearse the deploy *shape* while persistence waits for Phase 2. The harness also serves a browser UI at `GET /` (`client/testbed.html`) — the state-inspector lens plus Found-guild / Establish-venture / Tick / Reset controls — so the galaxy can be built up and advanced by clicking; the page computes nothing, it renders `/snapshot` and re-reads it after each action and each tick (refresh-on-action, manual tick, no polling).
...
*(Status note, updated 01-08-26. Consolidated to ONE generator: `generate_seed.js` now implements **all pipeline steps 1–10** — systems, naming, **ring classification**, planets **with ring-weighted archetypes and resource nodes**, outposts + Citadel, starter-eligible tagging, the **rare-tier repair pass**, and validation (`validation: PASSED` on seed 7331). Archetypes/nodes are applied as a decorating pass AFTER the base draws, so the system/planet/outpost skeleton stays byte-identical to the bare seed and `data/test_claims.json` still lines up. The former reference checkpoint `generate_seed_recovered_partial.js` is **removed** — its logic is absorbed here, with **Terran corrected to §2** (15 nodes incl. gold/silver/tungsten; the recovered copy wrongly had 9 and dropped the Molten metals), which also retires the old 5,290-vs-5,089 planet-count discrepancy (one generator, 5,290). Node **richness/yield is deliberately not generated** — an undecided number kept out of the canonical seed rather than placeheld. The rare-tier gradient (steps 4→5, added 02-08-26) measures ~38.7% inner / ~4.3% outer rare-tier on seed 7331 (9.0× spread) — close to §2's ~41%/~3.6% targets and exactly what the ×4 / ×1 / ×0.25 multipliers predict. The repair pass (step 9) fires **zero times** on 7331, and correctly applies **359** repairs when the floor is stress-raised from 2 to 50. **Starter count is now 368** (up from the blind-draw build's 337 — ring-weighting lifts Terran's effective probability across the far larger outer ring). Determinism re-verified (byte-identical re-run); the base skeleton is unchanged, so `data/test_claims.json` still lines up. One sub-decision is deliberately left open and flagged in code: *which* rare tier a repaired planet becomes (currently a base-weighted draw among the three) — unobservable on 7331 since the repair pass never fires there.)*
...
8. **Starter-eligible tagging** — ✅ *implemented.* A system qualifies with ≥1 Terran planet.
9. **Rare-tier repair pass** — ✅ *implemented.* Tops up any starter system short of its ≥2-rare-tier-within-⅓-radius guarantee by converting the nearest eligible non-Terran, non-rare-tier planets in range (deterministic: candidates ordered by distance, ties by planet id). On seed 7331 it fires **zero times** — the gradient alone already clears the bar, so this is a safety net for unlucky seeds, not the primary mechanism. Stress-tested by temporarily raising the minimum from 2 to 50, where it correctly applied **359** repairs and still validated. *(Which rare tier a converted planet becomes is a flagged open decision — see roadmap Track G — currently a base-weighted draw among the three; unobservable on 7331.)*
10. **Validation pass** — ✅ *implemented.* Asserts every `resourceType` appears somewhere in the galaxy, the starter pool is non-empty and spread across all 16 sectors (not just healthy in aggregate), and the rare-tier guarantee still holds post-repair. Results surface as `validation.passed` / `validation.issues` in the seed and on the command line. Not yet covered: illegal claim overlap.
...
- `tools/generate_seed.js` — the single generator, at **all pipeline steps 1–10** (incl. ring classification and the rare-tier repair pass): systems, naming, ring tags, planets **with ring-weighted archetypes and resource nodes**, outposts, the Citadel, starter-eligible tags, repair, and validation. Writes a **~3.4MB** seed for a 101,287-hex galaxy (1,500 systems, 5,290 planets, 24,137 resource nodes, 9 outposts). Stores no hexes. Guarded by `tools/generate_seed.test.js` (`node --test`): determinism, ring coverage, the rare-tier gradient band, the ≥2-within-⅓-radius guarantee, and the repair pass under stress. Determinism verified by running it: same seed → **byte-identical file**, different seed → different galaxy. The `generatedAt` field was dropped (ruling 20-07-26) — provenance is git's job — so the file is byte-identical across runs, not merely content-identical.
- `data/seed.json` — the reference seed (7331), now **populated**: planets carry `archetype` + `resourceNodes` and systems carry `starterEligible`. Regenerable, committed as the shared fixture the viewer, the claims tool, and future tests exercise. Its system/planet/outpost skeleton is unchanged from the bare seed, so `data/test_claims.json` still lines up.
...
- `data/test_claims.json` — reference claims (claims seed 4242) for the seed above: 20 guilds, 109 claimed systems, 60 guild outposts, 49 gates, 29 paths. `generatedAt` was dropped here too (ruling 20-07-26). One residual non-determinism source remains and is **still open**: the file embeds the input `seedFile` path string, which is environment-dependent (`./seed.json` vs an absolute path), so byte-stability holds only when it's generated the canonical way. Needs its own ruling — drop it, or store only the seed *number* it was built over.
- `client/seed_viewer.html` — loads a seed (and optionally a claims file) via file pickers and renders it. Territory fills at all zooms; true hex borders above 4× zoom (drawn live near the focus, since there's no stored lattice); hollow-ring "stargate" hexes for toll gates, dashed guild-coloured toll paths, deliberately identical diamond markers for outposts and gates; click any hex — system, guild outpost, toll gate, Syndicate outpost, Citadel, claimed territory, or empty space. Clicking a system's **OPEN DETAILS** opens an in-app **System View** (ported from the System & Planet Index mockup — since retired — into a `#detail-screen` overlay) that renders each planet's archetype and resource nodes from the loaded seed, with an 'Establish Venture' affordance on every node; exit returns to the map. The System View header now shows the system's **ring** (INNER/MIDDLE/OUTER), and the map-side info panel carries a Ring row for systems; both read `system.ring` from the seed.
...
*(The partial 15-07 generator once at `tools/generate_seed_recovered_partial.js` was consolidated into `generate_seed.js` and removed on 01-08-26. `system_planet_ui_mockup.html` — recovered 17-07-26 — had its System & Planet Index view ported into `seed_viewer.html`'s System View and wired to real seed data; the standalone file, left on hardcoded demo data, was retired and removed on 04-08-26 to prevent drift — see the status note above.)*

### The System & Planet Index UI

A **second client surface** alongside the galaxy map: a data drill-down for inspecting a system's planets and a planet's resource nodes, rather than a spatial view. Prototyped as a standalone mockup (`system_planet_ui_mockup.html`, retired 04-08-26) and now implemented as the **System View** in `seed_viewer.html` — plain HTML/CSS/JS, no build step, no framework, the same prototype-first posture as the map viewer.
...
**Known gaps** (the design now lives as `seed_viewer.html`'s System View, wired to real seed data — the retired standalone mockup's hardcoded demo data went with it): the right-hand content area is still an empty-state placeholder on every view (#35), and no real artwork exists yet (#36).
...
15. **Reputation inputs** — venture reputation must move on measurable events (missed deliveries, broken contracts), not vibes, or the takeover threshold becomes a dogpile cliff. *(Decided as a rule and now invariant 8; the exact event list is still unspecified — and, since the lease system, load-bearing for five mechanics: sharpened into #61.)*
...
23. **Lease dispute routing and venture relocatability** — ✅ _resolved_ (Section 5's lease treatment, 05-08-26): placed **assets are relocatable** (on lease termination the machine recovers via orbital limbo and the tenant hauls it home, §15.3), and lease **access is contractual, not a legality matter** — an active lease confers a standing right of access to the venture's site, so a tenant's presence is never a §7 trespass. Recorded as a decision: the landlord's leverage is **weaker** than §4's fixed-asset reading assumed, because the tenant can always leave with their machinery.
...
31. **Who should actually benefit from a fuel squeeze** — surfaced by the simulation (Section 8): a squeeze currently enriches an uninvolved steady producer *more* than the schemer who engineered it, while the intended victim suffers. Feature or problem? If a problem, the balancing lever is undecided. Also unresolved: whether the Manufacturer's shortfall should ever be survivable-but-degrading (the proposed survival floor, Section 8) rather than simply unmet demand. *(Addressed in principle 19-07-26: under the pooled-allocation model (§8) a big producer's big burn competes for a fixed pooled surplus and idle winnings buy less as the pool thins, so a windfall no longer compounds untouched. Whether that fully resolves the bystander-enrichment question needs the Phase 1 sandbox to show — the model isn't exercised until allocation is built.)* *(Further reframed by the refining monopoly, 05-08-26: the simulation assumed a guild legally refining and hoarding its **own** fuel; under the monopoly the schemer is either a legal buy-and-hold hoarder of **purchased** fuel or an outright criminal running a black-market reserve (§8) — no one can self-insure by legally stockpiling self-made fuel, which sharpens the squeeze.)*
...
34. **Wiring the System & Planet Index to real data** — ✅ _resolved (02-08-26)._ The Index's **System View** is wired to real seed data — ported into `seed_viewer.html` as a `#detail-screen` overlay, it renders each planet's `archetype`, its resource nodes, and the system's `ring` (INNER/MIDDLE/OUTER), with the map-side info panel carrying a Ring row too, all read from the loaded seed and verified in-browser (§16). The planet / resource / gradient work *is* now visible in a client. The one-time residual — the standalone `system_planet_ui_mockup.html` — has since been retired (04-08-26), its view living solely in `seed_viewer.html`; nothing further is outstanding.
...
38. **Salvage uniformity across Tier‑4 asset types** — does the −5% cut / 10% floor apply identically to structure-like assets (Toll Gate Kits, Factory Complexes) and actively-operated ones (ships, droids), or should they decay/salvage differently once the maintenance system exists?
...
48. **Force-sell — reframed by the refining monopoly (05-08-26).** No longer "keep or drop." Under the monopoly (§8; `fuel-allocation-model.md` §6/§9) there is **no legal guild fuel production**, so there is nothing to force-sell: the Phase-1 default is that **guilds don't refine fuel**, and legal hoarding exists only via *purchased* fuel. The old licence-premium anti-hoarding mechanism retires with it. Illegal refining is possible in fiction but unenforced until the §7 fine system (Phase 5).
...
51. **Quota-window fairness** — *no longer a fuel question* (the refining monopoly retires the fuel licence pledge, §8). The same concern survives for the **general venture-licence commitment window** (§5/D): checked over a window (24h placeholder), like the vote-window problem (#14), a guild that can't log in daily is punished. Window length interacts with tick rate and async play.
52. **Legitimate-dip grace** — *moot for fuel under the monopoly* (no guild fuel refinery/discount, §8); survives for the general venture-licence commitment: a venture knocked offline by a storyteller event shouldn't breach its commitment on the same tick, or the event does double damage; a grace/ramp is needed.
...
Added by the fuel-consumption correction (21-07-26):

54. **What burns Deuterium** — ✅ _resolved_: **only spacecraft, never ventures.** §3 previously read "consumed to run ventures and fleets," but that one clause contradicted the rest of the design: `docs/fuel-allocation-model.md` sizes every allocation to a *route's* fuel burn (never a venture's), the Phase 0 spreadsheet's per-guild "need" was an aggregate proxy tied to no venture, and `Vehicle.fuelCostToRun` (§15.4) is already the field for the mechanic. Ventures are now renewable-powered and consume no fuel; `Venture.energyDraw` is removed. The design intent this locks in: fuel is the price of *movement and interaction* (reaching other systems, the Syndicate, the market), so a guild content to operate entirely inside its own system needs no Deuterium — **deliberate isolation is a viable playstyle, not slow starvation.** Consequence for the fuel squeeze: scarcity bites through *fleet/route* burn and the allocation pool, exactly as the allocation model already describes, not through any venture-level draw.
...
**Added by the lease / LO-VO system (05-08-26):**

58. **Leased-venture tariff exemption.** A leased venture already pays the LO a lease fee and dividends; does the LO *also* collect §2's extraction/processing tariff on it? Three revenue streams against a tenant capped at 25% and expected to lose credits may be one too many.
59. **Venture valuation, and who funds the investor payout.** On dissolution, outside investors receive **the value of their shares** and nothing physical (assets go to their owner, §5). Open: how that value is computed — a pure-reputation figure evaporates the D3 capital injection the instant it is paid, so reputation may be better as a **multiplier on book value** than the value itself — and who funds it. Folds in §5's still-undefined *normal*-takeover payout: one question for both.
60. **The lease float — seller, capital recipient, and payout trigger.** The VO is locked at exactly 25% and cannot trade out of it; the LO's stake is a floor (26%+), not a fixed holding — so the 0–49% float looks like a *residual* the LO has not bought yet, with no defined seller. Who sells it, and who therefore banks the D3 capital injection? And does the dissolution payout fire on **non-renewal** as well as eviction — if only on eviction, the LO exits free by declining to renew; if on every ending, it is a flat tax on all leasing.
61. **The reputation event set — now load-bearing for five mechanics.** Takeover eligibility (§5), Syndicate forced closure (§5 lease), share value, dissolution payout size, and licence standing all key off reputation, yet #15 still lists the triggering event set as *unspecified* — now the most consequential undefined quantity in the design. Also needs: the takeover and forced-closure **thresholds and their ordering** (takeover above closure, so owned ventures are eaten by rivals on the way down while only leased ones survive to the Syndicate's cliff).
62. **Storyteller assistance anti-farming (incl. deliberate tanking).** Running a leased venture into the ground to cheapen the exit (§5) is precisely the kind of rapid-collapse signal a two-tailed storyteller (§9, §G to come) would *reward* — so a guild could farm assistance while cutting its exit liability. Needs a cooldown / derivative-signal guard; files with §9's anti-farming question.
...
4. **Track G, largely landed (02-08-26)** — the galaxy layer is implemented against §2/§13/§16: ring classification, the ×4 / ×1 / ×0.25 rare-tier gradient (~38.7%/~4.3% on seed 7331), the repair pass (zero fires on 7331), and validation; the viewer renders `archetype`, resource nodes, and `ring` (#34), and the Index UI (System View) is wired to real seed data. Seed 7331 yields **368** starter systems (the old 393 figure was a different, unrecovered build). Remaining Track G items are decisions, not mechanics — see roadmap.
