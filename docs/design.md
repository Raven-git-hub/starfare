...
**Gate 3 — under-supply distribution across consumers.** When the Downstream fork delivers **less than full demand** — because a higher-priority fork took the resource first, or the player capped downstream below full, or production simply fell short — that shortfall must be split across the **several consuming ventures**. The split is a player-set policy — **by percentage or by quantity** per consumer — with **no separate priority-rank field** (the allocation numbers *are* the priority). Where the player has set nothing, the engine falls back to the **default — first-come-first-served in venture-establishment order** (today's behaviour, now a documented, deliberate fallback). A **deterministic tie-break** is required (invariant 9). *(How Gate 3's per-consumer split is stored, and how it self-heals when it goes stale or over-commits, is the remaining **OPEN** item — see below. Gate 1's shape and semantics, above, are settled.)*
...
**The System Production Profile — where the settings live.** The gates are player-set parameters the tick must read, so they are **owned state**: a per-guild, **per-system** standing policy (one entry per good) that `stepProduction` consults every advance. For Gate 1 an entry stores, per good, the **fork ordering** and **each fork's amount** — the stockpile amount expressed as a percentage or a flat quantity at the player's choice, the downstream amount capped at full-tilt demand, the Syndicate amount capped at the licence minimum. Two principles are **settled**:
...
- **Self-healing by construction (Gate 1) / reconciled at read time (Gate 3).** Gate 1's waterfall cannot over-commit: it is recomputed every tick from the current forks and the current demand, so a venture added or removed simply changes the demand figure the Downstream fork pours into — there is nothing stored to go stale. The only place a stored setting *can* over-commit or name a vanished venture is Gate 3's per-consumer split; the mechanism that reconciles it deterministically each tick (filter the stale, default the silent, ration by order, flag for review) is the OPEN item below.

**OPEN — to settle in design before the interactive build slice (Gate 3 only):** the field shape for **Gate 3's shortfall split across consumers**, and the precise **self-heal / reconciliation mechanism** — what happens deterministically when that split names a removed venture, is silent about a new one, or its quantities exceed the shortfall. Gate 1 (the source waterfall) is **settled above** and can build now. Settle Gate 3, record it here, then prompt its slice.
