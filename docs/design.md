...
- `client/seed_viewer.html` — loads a seed (and optionally a claims file) via file pickers and renders it. Territory fills at all zooms; true hex borders above 4× zoom (drawn live near the focus, since there's no stored lattice); hollow-ring "stargate" hexes for toll gates, dashed guild-coloured toll paths, deliberately identical diamond markers for outposts and gates; click any hex — system, guild outpost, toll gate, Syndicate outpost, Citadel, claimed territory, or empty space. Clicking a system's **OPEN DETAILS** opens an in-app **System View** (ported from `system_planet_ui_mockup.html` into a `#detail-screen` overlay) that renders each planet's archetype and resource nodes from the loaded seed, with an 'Establish Venture' affordance on every node; exit returns to the map. The System View header now shows the system's **ring** (INNER/MIDDLE/OUTER), and the map-side info panel carries a Ring row for systems; both read `system.ring` from the seed.
...
*(Recovered 17-07-26 from the local machine and struck from the list above: `system_planet_ui_mockup.html`, now at `client/system_planet_ui_mockup.html`. The partial 15-07 generator once at `tools/generate_seed_recovered_partial.js` was consolidated into `generate_seed.js` and removed on 01-08-26 — see the status note above.)*
