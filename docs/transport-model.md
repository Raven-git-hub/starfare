...
- **The transport tab** and **the galaxy-map overlay** (client) — every active craft draws as a dot
  ...
  active shipment's current leg endpoints and its two ticks, and the client tweens. *(BUILT 10-09-26,
  Phase 2 — the **galaxy-map overlay** for the player's own in-flight Syndicate deliveries; see the
  AS-BUILT note in §3. The **transport tab** board stays Phase 4.)*
