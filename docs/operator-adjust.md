...
### 3.1 `adjustCredits { guildId, delta }`
...
### 3.3 `adjustGoods { guildId, systemId, good, delta }`
...
### 3.4 `grantAsset { guildId, kind, systemId }`
...
### 3.5 `removeAsset { guildId, assetId, occupied? }`
...
### 3.6 `removeVenture { guildId, ventureId, asset? }`
