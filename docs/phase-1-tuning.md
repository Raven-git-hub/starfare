...
|---|---|---|
| **Base price** | **per manufacturing tier** — T1 **1**, T2 **10**, T3 **100** (RULED 26-09-26) | The seed value every good starts at and the anchor the curve multiplies, now **tier-scaled** so a processed part is not priced as if it were raw ore. Keyed by `tierOf` (`sim/points.js`); ~~`deuterium` is tier 1 but never priced, so its base is moot.~~ **Correction, as built 26-09-26:** only `deuterium_fuel` is unpriced. Raw `deuterium` **is** priced — it has a posted price, which the licensed deuterium mine's per-tick auto-sale pays (`docs/fuel-supply-and-allocation.md` "Priced, but not hidden"; `sim/tick.js`) — so as a tier-1 good it takes the **T1 band**, and its resting price moves **10 → 1**. Whether that is intended is on the roadmap's decision checklist; nothing was special-cased. Supersedes the earlier uniform **10**. A ~10× step per tier is the value-ladder first cut (the human's numbers, not invented here); it is deliberately **decoupled from recipe input:output ratios** — see the refining-pump ruling, design.md §5 (Syndicate Exchange). |
...
| **Floor / ceiling** | **per manufacturing tier** — T1 **0.2 / 1000**, T2 **2 / 10,000**, T3 **20 / 100,000** (RULED 26-09-26) | Floor kept at today's **0.2× base** ratio; ceiling raised to **1000× base** for every tier. The clamp binds where `base × (1 + 0.05 × level) = ceiling`, so a uniform 1000× ratio puts the ceiling at **level ≈ 19,980** for all three tiers — ~53× further out than the old level-380 ceiling, making the cap a genuine *technical backstop* rather than a reachable target (its stated job). The old flat **2 / 200** is retired; note a flat floor of 2 would sit *above* T1's new base of 1, so a per-tier floor was mandatory, not optional. Real circuit-breakers remain the storyteller (Phase 6) and destabiliser/counter bots (Slice 7). |
