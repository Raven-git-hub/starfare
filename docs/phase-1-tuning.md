...
- **Syndicate commission-queue cap `SYNDICATE_QUEUE_MAX`** `[FIRST-CUT]` — **10**. The most Syndicate commissions a single guild may hold on `state.syndicateBuilds` at once — the WHOLE queue, the one building plus those waiting; an 11th `buyAssetFromSyndicate` is refused loudly at intake. Distinct from the dockyard's `MAX_QUEUE` (5): the Syndicate queue is per-GUILD (not per-dockyard) and more generous. `docs/asset-purchase.md` §"The queue cap".
