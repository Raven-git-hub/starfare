...
- **Syndicate craft speed** `[FIRST-CUT]` (29-08-26, retuned 14-09-26): **150 ticks per hex** (2.5 hours at 1 tick = 1 minute). A Syndicate BUY delivery travels the nearest-waystation→destination straight-line hex distance at this rate: `arrivalTick = buyTick + ceil(hexDistance × 150)`. A one-hex delivery lands in 2.5 hours, a four-hex one in under half a cycle — the friction that makes BUY a *planned* act against instant SELL. Distinct from **Transit ticks per edge** above (the guild-routed, per-leg number); the two should stay in sight of each other when the guild tier is tuned. Chosen as a first pass, to be felt against real deliveries and retuned; changing it is this one line. ⚠️ **Felt against real deliveries, 29-08-26 (the BUY engine slice) — and it is far too slow for the seed's hex scale.** Measured over seed 7331's 368 starter-eligible systems, the nearest-waystation distance is **6 hexes at the closest, ~60 median, 155 at the furthest** — so at 300 ticks/hex a delivery takes **1,800 to 46,500 ticks: 1.25 to 32 real-world days**. The number was ruled against the intuition "a one-hex delivery lands in 5 hours, a four-hex one in most of a cycle" (design.md §6), which assumes journeys of a handful of hexes; the generated galaxy is two orders of magnitude wider than that. Shipped as ruled at first, then **halved to 150 (14-09-26)** after a playthrough — a directional step, not the fix: at 150 the same distances give **900 to 23,250 ticks: ~0.6 to 16 real-world days** (median ~6). The durable fix is still open on the roadmap's decision checklist — the likely shapes are a much smaller ticks-per-hex, or a distance that is not raw hexes (a log or a per-ring band); both are rulings.
- **Syndicate hauler burn rate** `[FIRST-CUT]` *(fuel Slice 2, 31-08-26)*: **0.5 fuel units per hex** of route distance. A Syndicate trade's fuel cost is `fuelBurn = ceil(hexDistance × 0.5)`, measured from the system to its **nearest waystation** — the same distance the craft-speed line above times the flight on, read from the same `nearestWaystation` call so a trade cannot be priced on one geometry and flown on another. **Cargo-independent** (RULED, `docs/fuel-supply-and-allocation.md` §8): the hauler burns what the trip costs, not what it carries, so the burn does not move with the good or the quantity. `ceil` so a 1-hex route costs 1 rather than truncating to a free delivery. Defined once as `SYNDICATE_HAULER_BURN_RATE` in **`sim/fuel.js`**; nothing inlines it. **One hauler, one rate** — a per-craft table would be a table of one, and arrives with the guild transport tier (Phase 4). `design.md` §15.4 defines `Vehicle.fuelCostToRun` and names the Syndicate Hauler as an instance of it; no such Vehicle row exists in state, so this constant is the deliberate stand-in until one does. **Retune after the waystation rebalance** (9 → 96), which moves every distance in the galaxy and so moves every burn with it. Nothing spends fuel yet — Slice 2 only *quotes* it — so like `GUILD_STARTING_FUEL` it cannot be tuned honestly until Slice 3 makes it a real cost. **⤳ REFINED 11-09-26 — the single hauler becomes three tiers (the multi-good BUY / tiered-hauler ruling).** The Syndicate hauler now has **three capacity+burn tiers**, chosen by the **total units on a leg** (Σqty across all goods; integer goods): **light ≤ 10,000 @ 0.5/hex**, **medium ≤ 50,000 @ 0.6/hex**, **heavy ≤ 200,000 (the cap) @ 0.7/hex** — **Option C**, burn rises per hex with the tier; caps and rates ruled by the human 11-09-26. The light rate IS the old flat `SYNDICATE_HAULER_BURN_RATE = 0.5`, so a small leg is unchanged; the constant becomes the light row of a rate/cap table. **The "cargo-independent" claim above is refined, not dropped:** burn is still flat *within* a tier (it does not vary with the good, nor move continuously with quantity), but *which* tier applies is chosen by the leg's total units — a step function, not a smooth curve. A leg over the heavy cap is **reject-whole** (split the order). The mechanic (shape, why, every-leg BUY+SELL) is `transport-model.md` §5.1 + §8.0; this entry stays the authority on the **numbers**, still `[FIRST-CUT]` and awaiting the same waystation-rebalance retune. **⤳ REVISED 14-09-26 — capacity is cargo SPACE, and the caps move.** Capacity is no longer unit-count but **space** = `Σ (qty × volume)`, with **volume by manufacturing tier** (`tierOf`): **T1 = 1**, **T2 = 100**, **T3 = 60,000**, **T4 non-movable asset = 6,000,000** (a whole heavy hold). The holds become **space** caps: **light 10,000 / medium 50,000 / heavy 6,000,000** — the heavy hold moved **200,000 → 6,000,000**, so a heavy carries 100 T3 modules, or one T4 asset, or 6,000,000 raw. Rates unchanged (0.5 / 0.6 / 0.7). The human's anchors fixed the ratios (100 T2 = a light; a T3 will not fit a medium; 100 T3 = a heavy; a T4 asset = a heavy hold); the scale is `[FIRST-CUT]`. **The T4 volume already binds:** a bought asset (2.1d, `buyAssetFromSyndicate`) ships on the §6 rails at the **heavy** rate per §5.1, superseding asset-purchase.md's light-rate placeholder (the light→heavy code change rides the tiered-hauler build). Shape: `transport-model.md` §5.1.
...
### The build yard (dockyard) — Tier-4 build core *(13-09-26 — roadmap 2.1b slice 1, `docs/build-yard.md` §3; `sim/asset-recipes.js`)*
The dockyard's build core carries exactly two new numbers, both `[FIRST-CUT]` and **RULED this design session (13-09-26)**. They live once in `sim/asset-recipes.js` and are imported, never inlined (`sim/tick.js`, `sim/actions.js`); this table is the authority on the values. Retune in play — a build yard has never been felt.
...
- **Syndicate commission-queue cap `SYNDICATE_QUEUE_MAX`** `[FIRST-CUT]` — **10**. The most Syndicate commissions a single guild may hold on `state.syndicateBuilds` at once — the WHOLE queue, the one building plus those waiting; an 11th `buyAssetFromSyndicate` is refused loudly at intake. Distinct from the dockyard's `MAX_QUEUE` (5): the Syndicate queue is per-GUILD (not per-dockyard) and more generous. `docs/asset-purchase.md` §"The queue cap". *(**✅ BUILT 26-09-26** — defined once as `SYNDICATE_QUEUE_MAX` in `sim/asset-recipes.js`, read by the `buyAssetFromSyndicate` gate; pinned by `sim/tests/syndicate-queue-cancel.test.js`.)*
- **Build times `BUILD_TICKS` (ticks per kind)** `[FIRST-CUT]` — **miner 720** (12 hours), **factory 960** (16 hours), at the ruled 1,440 ticks/day (`docs/cycle-and-calendar.md`). Counted down *after* the whole module bill is consumed atomically (reserve-and-wait, `build-yard.md` §3): the consume tick sets the countdown, and the finished asset is emitted exactly `BUILD_TICKS` ticks later, idle at the dockyard's system. Only the two buildable kinds have a value; the ship / outpost / scanner / toll-gate / droid ladder (`build-yard.md` §3 notes "light transport shortest → outpost / deep-scanner longest") is deferred with those outputs — **EXCEPT the ship ladder, which now enters the build** (the four transport classes; see "Guild transports" below). The **module BILLS** themselves are not numbers ruled here — they are lifted verbatim from `docs/asset-recipes.md`'s two `buildable` rows into `sim/asset-recipes.js`, and a load-time tripwire halts if any bill names a good that is not a real Tier-3 module.
...
### Guild transports — the guild transport tier (2.2-foundation) *(18-09-26 — the guild-transport thread; design.md §6 / §15.4, `docs/transport-model.md`)*
The four guild-owned craft classes, all `[FIRST-CUT]`, RULED this design session (18-09-26). A guild transport is a **`Vehicle`** (design.md §15.4), built by a dockyard or bought from the Syndicate exactly as a ground asset is (the buy/build machinery is kind-general), minted **idle** into `guild.vehicles` at the building / delivery system. This table is the authority on the numbers the build slice reads. Retune in play — a guild transport has never been felt.

> **BUILT — 2.2-foundation engine slice (a), engine + snapshot only (18-09-26; `sim/vehicles.js` the class vocabulary + `VEHICLE_SPECS`, `sim/state.js` `createVehicle` gains `systemId`/`maintenanceCondition`, `sim/asset-recipes.js` the four ship bills + `BUILD_TICKS` + `VEHICLE_BUY_BASELINE`, `sim/actions.js` buy/build accept the four classes, `sim/tick.js` the shared `mintFinishedKind` seam, `sim/snapshot.js` the per-guild `vehicles` rows + purchase quote, `sim/invariants.js` `checkVehicleIntegrity`; tripwires `sim/tests/vehicles.test.js`).** Every number above is read from this table and **none was invented**. A guild can now BUY (Syndicate) or BUILD (dockyard) one of the four classes; it is minted **idle** into `guild.vehicles`. **No movement, no cargo, no client** — dispatch/routes are slice (b), so `capacity`/`defenseRating` are carried but read by nothing. Two build-time PINS, both consistent with the existing asset-delivery discipline rather than new numbers: the delivery burn is `ceil(fuelCostToRun[class] × hexDistance)` and the flight is `arrivalTick = buildDoneTick + ceil(hexDistance × speed[class])` — the SAME `ceil` any Syndicate delivery already rounds a part-tick / part-unit up with (`transport.js` / `fuel.js`), so every real leg costs at least 1 fuel and lands on a whole tick. The `/asset-recipes` HTTP endpoint's `buildable` list still advertises only `[miner, factory]` (its `bills` already carry the ship bills) — wiring the client's transport catalog is a later (client) slice, so the endpoint's advertised vocabulary was left for it; the engine gates accept the four classes.

- **Per-class stats.** `speed` is **ticks per hex** (LOWER is faster, the same unit as the Syndicate craft's `CRAFT_SPEED = 150`); `capacity` is cargo **space** (`Σ qty × volumeOf`, the hauler-tier unit); `fuelCostToRun` is the craft's **own** per-hex burn (per-craft now — only Light still coincides with the Syndicate light hauler's 0.5); `defenseRating` is **inert until combat exists** (placeholders, any ordering).

  | Class | `speed` (ticks/hex) | `capacity` (space) | `fuelCostToRun` (/hex) | `defenseRating` | Buy baseline (credits) | Build (`BUILD_TICKS`) |
  |---|---|---|---|---|---|---|
  | `lightTransport`  | **105** (0.7 × 150; ~30% less travel time than Syndicate) | **10,000** | **0.5** | 10 | **5,000,000** | **360** (6 h) |
  | `mediumTransport` | **150** (= Syndicate) | **50,000** | **1.0** | 20 | **15,000,000** | **960** (16 h) |
  | `heavyTransport`  | **225** (1.5 × 150; 50% more travel time than medium) | **6,000,000** | **100.0** | 40 | **200,000,000** | **10,080** (1 week) |
  | `spycraft`        | **26.25** (105 ÷ 4; 4× faster than light) | **0** (no cargo) | **20.0** | 5 | **100,000,000** | **10,080** (1 week) |

  Speed convention: "faster" = fewer ticks per hex. Spy's 26.25 is the exact 105 ÷ 4; the float is fine (`arrivalTick = tick + ceil(distance × speed)` handles it) — round to 26 only if integer ticks-per-hex is later wanted. `capacity 0` on `spycraft` means it structurally **cannot** be handed cargo (espionage craft, not freight). The `defenseRating` column is pure placeholder — nothing reads it until combat.

- **Heavy fuel is a deliberate gate, not a slip.** Heavy's **100/hex** is ~140× the Syndicate heavy rate and 100× Medium — intended: because a bought craft flies itself in at its **own** burn (below), a 10-hex heavy delivery costs 1,000 fuel against a 500 starting hoard (`GUILD_STARTING_FUEL`), so a *bought* heavy is a serious late-game commitment. The free starter heavy (a later starting-conditions tune, at 50% maintenance) sidesteps this; a bought one does not.

- **Buy price — a per-class baseline, not the flat floor.** A guild transport is priced by the SAME formula as a ground asset — `price = max(baseline[class], round(partsCost × ASSET_PURCHASE_REDUCTION))` — but the flat `ASSET_PURCHASE_FLOOR` (12M) is replaced **per class** by the baselines above (5M / 15M / 200M / 100M). Miner/factory keep the 12M floor. At today's parts scale the baseline binds for every class (parts are near-nothing), exactly as the 12M floor binds for miner/factory now, so the `partsCost × 0.8` branch stays live-but-dormant for later tuning. The ship module **BILLS** are already in `docs/asset-recipes.md` (the four `buildable` ship rows) and are lifted verbatim into `sim/asset-recipes.js` by the build slice, like the miner/factory bills.

- **Delivery — the craft flies itself in, on its own clock and its own fuel** (supersedes "Delivery fuel reuses the light hauler tier" above, **for vehicles only**). A bought craft is not carried on a Syndicate hauler: after the `BUILD_TICKS` build it flies from the nearest waystation to the destination at its **own** `speed`, and the buyer pays its **own** `fuelCostToRun × hexDistance` up front (the same "pay the burn at dispatch" discipline as any asset, but at the craft's rate, not the heavy-hauler rate). So `deliveryTick = buyTick + BUILD_TICKS[class] + ceil(hexDistance × speed[class])`. Ground assets (miner/factory) are unchanged — they can't fly, so they still ride the light-hauler delivery of the 2.1d rule above.

### The guild Outpost — capacity & dock slots (2.2, the outpost ladder slice 1) *(20-09-26 — design.md §4 "The Guild Outpost"; `sim/outposts.js`)*
Two numbers, both living once in `sim/outposts.js` and imported, never inlined; this table is the authority on them. A guild Outpost is a **SHARED, single-hex** infrastructure claim in the Toll Gate family (design.md §15.4), placed/torn-down by the operator this slice. Both values are **CARRIED but INERT** here — nothing enforces or consumes them until the cargo/dock model (slice 3).

- **`OUTPOST_CAPACITY`** — the stockpile's hard space cap, in cargo space (`Σ qty × volumeOf`, the hauler-tier unit). **RULED 20-09-26: `= 30 × HEAVY_HOLD`** — thirty heavy-hauler holds (design.md §4 "Capacity"). It **invents no number**: it is DERIVED from `sim/fuel.js`'s `HEAVY_HOLD` (itself derived from the `HAULER_TIERS` table), so the cap and the hauler tier can never disagree. At today's `HEAVY_HOLD = 6,000,000` that is 180,000,000 space (~30 stored assets, since one non-movable T4 asset fills one heavy hold, or the goods-volume equivalent, mixed freely). Retune only by re-ruling the "thirty holds" multiple, never by typing the product.
- **`OUTPOST_DOCK_SLOTS`** — the number of docking slots (design.md §4 "The dock model"). `[FIRST-CUT]` — the doc's working value **10**. A slot count, a throughput throttle (the doc's deadlock-free dock model), not felt yet; retune when the dock model lands.

- **`OUTPOST_DOCK_TURNAROUND` — per-class load/unload time (ticks a craft holds a slot)** — **RULED 21-09-26** (design.md §4 "The dock model"): **light 5, medium 30, heavy 120** ticks. Chosen in real time at the live 1-tick = 1-minute clock (5 min / 30 min / 2 h) and stored as **ticks** (the engine's unit), so if the tick-duration mapping is ever formalised these are the numbers to revisit. It is the whole dock cycle's cost — one manifest, however many goods — and **Outpost-only**: a transfer at a guild's own system is instant. It does **not** apply to Syndicate craft (they never dock), and `spycraft` (capacity 0, carries nothing) never transfers. Lives once in `sim/outposts.js`, imported never inlined.

> **BUILT — the entity + operator spawn/remove, engine + snapshot + operator CLI (2.2, the outpost ladder slice 1).** `OUTPOST_CAPACITY` and `OUTPOST_DOCK_SLOTS` above are the only two numbers this slice adds, and neither is invented (the cap is derived, the slot count is the doc's sourced working value). The entity carries them and its `createdAtTick`; `capacity`/`dockCapacity`/`stockpile` are read by nothing (no cargo/dock behaviour — slice 3). Placed FREELY by the operator (`spawn-outpost`), exactly as `spawn-vehicle` places a craft — placement range and anchor-ownership are the real build/deploy slice's. A NO-OP on a galaxy with no outpost (omit-when-empty `state.outposts` / `guild.outpostSerial`, goldens byte-identical). Sim suite 1,367 → **1,383 green**, `tools/admin.test.js` 32 → 35.

> **BUILT — the Outpost dock model (park / queue / slot / turnaround), engine + snapshot + operator CLI (2.2 cargo, engine slice 2, 21-09-26; design.md §4 the OUTPOST half).** All three numbers here are now LIVE and none is invented: `OUTPOST_DOCK_SLOTS` (10) throttles concurrent transfers, `OUTPOST_DOCK_TURNAROUND` (light 5 / medium 30 / heavy 120) times each slot, `OUTPOST_CAPACITY` (30 × HEAVY_HOLD) hard-caps the stockpile an unload clamps to — all sourced above, imported from `sim/outposts.js`, never inlined. A transfer at a guild's own Outpost queues for a slot and resolves after its class turnaround (goods move at completion); the Outpost stockpile now fills and counts in Galactic Supply; teardown destroys the stored goods and evicts docked craft. `spycraft` has no ruled turnaround (capacity 0), so a dock transfer of one is refused rather than a number invented. Sim suite 1,398 → **1,412 green** (the new `sim/tests/outpost-dock.test.js`, +14).
...
|---|---|---|
| **Base price** | **per manufacturing tier** — T1 **1**, T2 **10**, T3 **100** (RULED 26-09-26); raw `deuterium` excepted, keeps **10** | The seed value every good starts at and the anchor the curve multiplies, now **tier-scaled** so a processed part is not priced as if it were raw ore. Keyed by `tierOf` (`sim/points.js`); ~~`deuterium` is tier 1 but never priced, so its base is moot.~~ **Raw `deuterium` is EXCEPTED (RULED 26-09-26):** it is out of the tier system (design.md §8) — `tierOf`'s "tier 1" is only the Points view — so it is **not** re-tiered. It stays priced (the licensed deuterium mine's auto-sale pays its posted price) on its **status-quo** band, **base 10 / floor 2 / ceiling 200** (`DEUTERIUM_BAND`, `sim/prices.js`). A separate fuel-facing deuterium price is a future fuel-economy decision (`docs/fuel-supply-and-allocation.md` §1.4). Supersedes the earlier uniform **10**. A ~10× step per tier is the value-ladder first cut (the human's numbers, not invented here); it is deliberately **decoupled from recipe input:output ratios** — see the refining-pump ruling, design.md §5 (Syndicate Exchange). |
| **Level sensitivity** | **0.05** | The level is *ticks of galaxy-wide production held in stockpiles* (`stock ÷ capacity-per-tick`), so this sets the timescale on which hoarding pays: a level of 20 doubles the value. The old flat ceiling (20× base) needed a level of 380 — a little over **six hours** of wholly unmanaged hoarding at the ruled 1 tick = 1 minute; the per-tier ceiling (1000× base, RULED 26-09-26) needs **~19,980** (see *Floor / ceiling*). **Chosen against a live run**: the first value tried (0.6) pinned every hoarded good at the (then 20× base) ceiling inside forty ticks, which is a flat line, not a market. |
...
| **Floor / ceiling** | **per manufacturing tier** — T1 **0.2 / 1000**, T2 **2 / 10,000**, T3 **20 / 100,000** (RULED 26-09-26); raw `deuterium` excepted, keeps **2 / 200** | Floor kept at today's **0.2× base** ratio; ceiling raised to **1000× base** for every tier. The clamp binds where `base × (1 + 0.05 × level) = ceiling`, so a uniform 1000× ratio puts the ceiling at **level ≈ 19,980** for all three tiers — ~53× further out than the old level-380 ceiling, making the cap a genuine *technical backstop* rather than a reachable target (its stated job). The old flat **2 / 200** is retired for every manufacturing good — raw `deuterium` alone keeps **2 / 200** (excepted, see *Base price*); note a flat floor of 2 would sit *above* T1's new base of 1, so a per-tier floor was mandatory, not optional. Real circuit-breakers remain the storyteller (Phase 6) and destabiliser/counter bots (Slice 7). |
...
*(**AS-BUILT 26-09-26 — per-tier price bands.** `PRICE_BANDS` in `sim/prices.js` carries the two rows above exactly — `{ 1: { base 1, floor 0.2, ceiling 1000 }, 2: { base 10, floor 2, ceiling 10,000 }, 3: { base 100, floor 20, ceiling 100,000 } }` — keyed by `tierOf`, beside `DEUTERIUM_BAND` `{ base 10, floor 2, ceiling 200 }` for raw deuterium (excepted, RULED 26-09-26 — its price row is byte-identical to before the slice), and replaces the flat `BASE_PRICE` / `PRICE_FLOOR` / `PRICE_CEILING` (retired; no longer exported). `bandFor(good)` / `basePriceFor(good)` read a good's row (null for a non-priced good, as before) and **throw, naming the good,** if a priced good ever resolves to a tier with no row. A fresh galaxy seeds each good at its tier base; the target, the zero-capacity rest and the clamp all use the good's own band; the `sim/invariants.js` price-sanity tripwire checks each good against its own band. No other price constant moved. Tripwires: `sim/tests/prices.test.js` (the ruled table pinned, every priced manufacturing good has a tier band, deuterium on its own 10 / 2 / 200 band and not T1's, seeded 1 / 10 / 100, rests at its tier base, clamps at 1,000 / 10,000 / 100,000 and 0.2 / 2 / 20, the per-tier invariant). **A fresh galaxy is required** — a live galaxy's stored price rows are not migrated. Note, not a new decision: the tick's formula cannot drive a price below **0.5 × base** (the lowest idleness factor at level 0), so the floor, like the ceiling, is a backstop the formula never reaches; it binds only on a hand-set or corrupted value. That was equally true of the old flat floor.)*

**Open, for the human — not invented here:** **per-good** base prices *within* a tier (the per-**tier** ladder is RULED 26-09-26 above; a finer per-good base is a possible later refinement); per-recipe baseline outputs (per-resource mine yields are RULED 24-09-26, "Resource yield tiers" below); whether the level curve should stay **linear** in the
...
- **Market** — ~~`[FIRST-CUT]`: reuses the fuel price curve's *shape* (§5) with its own levers — base $3,
  ...
  of its own: every non-fuel good is priced by the one curve (base **per manufacturing tier** — titanium, a T1 raw, rests at **1** since 26-09-26, was **10** — level sensitivity **0.05**,
...
- **Client establish-rate** `[FIRST-CUT]` *(client-wiring Slice 3, 24-08-26)*: the `productionRate` the **game client** sends with every `establishVenture` — a uniform **5**, for every resource and for refineries alike. Given by the human for the slice and explicitly *to change*. It does **not** change the engine's per-resource intent below: `establishVenture` takes whatever rate the caller sends and stores it, so this is the client's single stand-in until the per-resource extraction rates (and a refinery throughput rule) are wired through the establishment panel. Defined once as `ESTABLISH_RATE` in `client/game.html`; a venture seeded by the operator over `POST /action` still carries whatever rate that call names. *(**SUPERSEDED 24-09-26** by design.md §2 "Resource Yield Tiers & the Homeworld Production Floor": the client stops sending a rate for mines and factories, and the engine stamps `productionRate` from the venture's baseline. `ESTABLISH_RATE` survives only where no baseline exists (the deuterium refinery). Marked AS-BUILT when the build slice lands.)* *(**AS-BUILT 24-09-26.** `establishVenture`'s `productionRate` is optional: omitted, the engine stamps the venture's baseline (`baselineRateFor`, `sim/baseline.js` — a mine's `MINE_BASELINE` in units/tick, a factory's `REFINERY_BASELINE` in batches/tick) and refuses if there is none. The client's general establish (mining + refining) and its deuterium-mine establish send no rate; the establishment ledger's Rate row shows the baseline served by `GET /goods` (`mineBaseline` / `refineryBaseline`). `ESTABLISH_RATE` = 5 remains ONLY on `establishDeuteriumRefinery`, whose throughput is unruled. An operator's `POST /action` may still name any positive rate.)*
...
| `REP_MEET_MAX` / `REP_W_COMMIT` / `REP_W_EQUITY` | `[FIRST-CUT]` **10 / 0.5 / 0.5** *(rescaled 01-09-26)* | Met-cycle gain = `REP_MEET_MAX · tierFactor · (REP_W_COMMIT · committedOutputPct + REP_W_EQUITY · equityFrac)`. **Rescaled 100 → 10** (÷10, matching the RP scale) and gains a **`tierFactor = W_TIER / 100`** (T1 = 1, T2 = 1.5, T3 = 3, T4 = 5) so a higher-tier venture earns proportionally more per cycle and *every* tier breaks even in the same **~10 cycles** — higher tiers are a reward (more GP, more earning, bigger dividend), not a heavier bar. Full-terms T1 → **+10/cycle**; a mine (bar 100) breaks even in ~10 cycles ≈ 10 game-days. `equityFrac` unchanged. Shape in `points-and-reputation.md §2.6`. **✅ BUILT 01-09-26** (`sim/licence.js` `metGain`, with `tierFactor` — see the AS-BUILT note below). **⤳ RULED 06-09-26 — tier no longer scales the per-cycle earn.** `tierFactor` is DROPPED from `metGain`: a market venture's met gain is `REP_MEET_MAX · (REP_W_COMMIT · commit + REP_W_EQUITY · equityFrac)`, the same for every tier — full terms **+10/cycle** whatever the tier. A higher tier's reward is its **market price** (higher-tier goods sell for more), plus its bigger signing bump and GP — NOT a faster reputation climb; reputation is trust earned by behaviour, not by size (the old “every tier breaks even in ~10 cycles” rationale retires with the tier scaling). **Deuterium is the deliberate exception** — `deuteriumMetGain` keeps `tierFactor(T4)` = 5 → +50/cycle, because deuterium sells nothing and reputation is its only reward channel (§1.4). The shape in `points-and-reputation.md §2.6` is updated in the SAME commit as the code. |
| `REP_BREACH_MAX` / `REP_BREACH_MIN` | `[FIRST-CUT]` **10 / 2.5** *(rescaled 01-09-26)* | Breach drop = `−(REP_BREACH_MAX − (REP_BREACH_MAX − REP_BREACH_MIN) · committedOutputPct) · tierFactor` — still linear **inverse**-commitment (a high-committer who narrowly fails is barely penalised; the Syndicate does not punish maximum effort, which already costs the guild all its output). **Rescaled 100/10 → 10/2.5:** `MAX` ÷10 with the scale, and `MIN` **lifted from 10% to 25% of MAX** so the gentlest breach (100% commit) is `−2.5·tierFactor` (T1 ≈ −3/cycle), not −1. This blunts the sign-max-grab-the-bump-then-breach exploit (free-boost tail ~100 → ~34 cycles) *without flipping the curve*; the full undiscounted licence fee a breach already pays is the rest of the deterrent. 25%-enough is a playtest question. Shape in `points-and-reputation.md §2.6`. **✅ BUILT 01-09-26** (`sim/licence.js` `breachPenalty`). As built, a tier-1 venture at 100% commitment pays **−3**/cycle (2.5 rounded); at tier 2, −4. **⤳ RULED 06-09-26 — tier no longer scales breach either.** For symmetry with the earn side, `tierFactor` is DROPPED from `breachPenalty` too: every market venture breaches on one curve `−(REP_BREACH_MAX − (REP_BREACH_MAX − REP_BREACH_MIN) · commit)`, so a 100%-commit breach is **−3/cycle at every tier** (was −4 at T2). The ladder is tier-blind up AND down — a bigger venture is not punished harder for the same failure; its stake is already bigger through its GP and bump. Deuterium is breachless, so untouched. |
| `W_SYS` / `W_T1` / `W_T2` / `W_T3` / `W_T4` (GP weights) | `[FIRST-CUT]` **200 / 100 / 150 / 300 / 500** *(rescaled 01-09-26)* | `GP = W_SYS·(held systems) + Σ ventures W_TIER(tier)`. **Rescaled from 12/6/8** to a small-integer scale where GP = expected RP directly. A held system stays **2× a tier-1 mine** (200 vs 100) — the anti-sprawl bite preserved (empty territory is pure GP with no RP to pay it back). Tier spread widened to a deliberate **1 : 1.5 : 3 : 5** to reward climbing the tree. Integer GP; DERIVED, never stored. See `points-and-reputation.md §1.0 / §2.6`. **✅ BUILT 01-09-26** (`sim/points.js` `W_SYS` / `TIER_WEIGHT`). ⚠ **It forced `BASE_GRANT_PER_GP` 5 → 0.3** — see the fuel section's AS-BUILT note. **⤳ `W_T3` = 300 WENT LIVE 2.1a** (the Tier-3 module catalog, `docs/asset-recipes.md`): the 25 module goods are now real and producible, so `tierOf` resolves a module to tier 3 and `TIER_WEIGHT` gained `3: 300`. A venture manufacturing a module now SCORES 300 across GP / issuance / mean line / signing bump instead of halting — the halt was a latent cross-system crash reachable the moment the client's Tier-3 grey-out (a UI guard, removed next slice) stopped hiding it. Applies the already-ruled 3× rung; invents nothing. **`W_T4` stays `[DEFERRED]` as a GP weight** — no Tier-4 good exists; its only reader is the deuterium-licence RP path (still an unweighted GP tier halts). |
| **Venture signing bump** `[FIRST-CUT]` **`2 · committedOutputPct · W_TIER`** *(new 01-09-26)* | On **signing a licence**, the venture is minted with an instant RP bump = `2 · committedOutputPct · (its GP weight)`. `0%` commit → **0** (flatlines, below its line); `50%` → **1× its GP** (bar-neutral, on its line — the Syndicate's expected deal); `100%` → **2× its GP** (its GP *above* the line, instant fuel buff); linear between. Generalises §1.2's passive-asset offset to **productive** ventures and scales it by negotiated terms, so the deploy slider sets whether a venture launches above / on / below its line. Lives on `venture.reputation` at signing (so `checkGuildReputationSum` covers it, no new term); **supersedes** the old "ventures earn their whole bar from zero." Shape in `points-and-reputation.md §2.6`. **✅ BUILT 01-09-26 (slice 2 of 2)** — `sim/licence.js` `signingBump`, minted by the `applyForLicence` apply in `sim/actions.js`. Verified at founding scale: a home system + one tier-1 mine is GP 300 / expected 300, and signing that mine at **50%** puts the guild at `endowment 200 + bump 100 = 300` — modifier **exactly 1.00**, on its line; at 100%, RP 200 and above the line; at 0%, no bump and below it. A zero bump mints **no key at all**, exactly as a zero-delta verdict does in the tick. See the AS-BUILT note below for the one sharp edge it surfaced. **⤳ (06-09-26) The bump KEEPS its tier scaling** even though the per-cycle earn/breach lost theirs: the bump is sized to the venture's own GP (the bar it just added), so a deploy stays bar-neutral — that is a size question, not a reward, and dropping it would make deploying a bigger venture an instant reputation hole. Earn rate is tier-blind; opening position is not. |
| **Deuterium licence RP** `[FIRST-CUT]` *(slice 2, ruled 04-09-26)* | The windowless deuterium licence (fuel-supply §1.4) earns RP on its own cadence, reusing existing constants at the **T4** weight: a **1000 RP signing bump** (`signingBump` = `2 · 1.0 · W_T4`, 100% commit) minted by `licenseDeuteriumMine`, and a **+50 RP/cycle** automatic MET (`REP_MEET_MAX · tierFactor(T4)` = 10 · 5) — **equity-free and breachless** (delivers continuously, can never breach; commitment implicit 100%, equity removed). This **pulls `W_T4` = 500 into `TIER_WEIGHT`** (was `[DEFERRED]`) because deuterium needs the T4 lookup; `W_T3` stays out and an unweighted tier still halts. The bump is a **deliberate windfall** — deuterium adds zero GP, so there is nothing to offset; its value IS the reputation. Numbers ride the band-rescale ruling below. |
| **Dockyard RP signing bump** `DOCKYARD_SIGNING_BUMP` `[FIRST-CUT]` **900** *(roadmap 2.1b slice 2, ruled 13-09-26; `docs/build-yard.md` §5)* | The **held Tier-4 RP signing bump** a build yard (`isDockyard`) is minted with on **establish** — a **flat 900**, minted ONCE by the `establishDockyard` apply onto `venture.reputation` with `guild.guildReputation` tracking it (so `checkGuildReputationSum` covers it, no new term). Not `2 · commit · W_T4`: a dockyard commits nothing, it *exists*, so the bump is a flat standing value, not terms-scaled. **NO per-cycle accrual** (per-cycle is the deuterium mine's alone). **Held while it stands, forfeited on teardown → not farmable.** Sized as the Tier-4 counterpart of the deuterium mine's 1000 but **deliberately below it**, because the dockyard also adds **+500 GP** (a Tier-4 GP special-COUNT, the `TIER_WEIGHT[4]` = 500 row above — the mine's 0-GP is its unique advantage): with `MEANLINE_K` = 1 that +500 GP raises the bar by 500, so the dockyard's **net standing benefit = 900 − 500 = +400**. Lives once in `sim/licence.js` (the RP layer owns RP magnitudes, §4); **✅ BUILT 13-09-26** (`sim/licence.js` `signingBump` special-cased on `isDockyard`; applied in `sim/actions.js` `establishDockyard`). Retune in play — a build yard has never been felt. |
...
**The Tier-4 net-benefit tuning invariant (roadmap 2.1b slice 2, ruled 13-09-26; `docs/build-yard.md` §5).** The dockyard's two `[FIRST-CUT]` numbers — the **+500 GP** Tier-4 special-COUNT (`TIER_WEIGHT[4]`, reused) and the **900 RP** signing bump (`DOCKYARD_SIGNING_BUMP`, new) — are set **together** to satisfy a *relation*, not chosen as lone values. With `MEANLINE_K` = 1 a venture's **net standing benefit** on deploy is `signingBump − GP added` (the +GP raises the bar by the same amount), and the numbers order the three Tier-4-capable deploys so:

> **deuterium mine (0 GP + 1000 RP → +1000) > dockyard (+500 GP + 900 RP → +400) > the best Tier-1/2/3 production venture (tier-3 at 100 % commit: 300 GP + 600 RP → +300; 0 at the neutral 50 %).**

The deuterium mine keeps its unique 0-GP advantage (its whole bump is net standing); the dockyard is a genuine size increase whose bump is tuned to sit *below* the mine and *above* an ordinary top-tier venture. This ordering is the ruling — neither the 500 nor the 900 is defensible alone, and it is pinned as a test (`sim/tests/dockyard-points.test.js`). Retune the pair together, never one in isolation.

**RP band — RULED 06-09-26: one universal, absolute ladder, kept as-is (NOT rescaled, NOT per-tier).** The bounds (`RP_FLOOR` −500, `RP_TAPER_KNEE` 800, dividend-max 1000, `RP_SOFT_CAP` 1500) stay **the same fixed values for every venture at every tier**. The 1000 dividend-max and the −300 / −500 consequence marks are semantic anchors — a venture either earns trust or it does not, and what tier it is does not change that. The “one global band vs a per-tier band” sub-choice is resolved: **global.** The known consequence is **INTENDED, not a bug**: because RP is an absolute-scale stock while expected RP (= GP) scales with the guild, a mature, well-run guild's summed RP outruns its GP, so its issuance modifier climbs to the **1.5 ceiling and rests there.** The mean line still does its load-bearing work at equilibrium — it ramps the young and penalises sprawl (the founding endowment is stamped ONCE and does not grow with systems acquired later, so grabbing territory without deploying still sinks the ratio). What it stops doing is separating great mature guilds from merely-good ones; that finer discrimination is deliberately **deferred to richer RP-earning avenues in later phases**, where the interesting large-guild choices will live, rather than bought by rescaling the band now. Do **not** per-tier the band and do **not** decouple `MEANLINE_K` from GP — both were considered and declined (06-09-26).
...
| Constant | `[FIRST-CUT]` | Rationale |
|---|---|---|
| `BASE_GRANT_PER_GP` | **0.3** *(re-based 01-09-26; was 5)* | Fuel per GP per cycle at modifier 1.0. `fuelGranted = round(BASE_GRANT_PER_GP · GP · modifier)`. A par guild (GP 500, modifier 1.0) draws ~150/cycle; a big well-run one more; an over-expanded one its base × 0.3. **BUILT 31-08-26 (slice 5a)**, `sim/issuance.js` `grantFor`. Verified: a par guild of GP 500 on its line draws **149**/cycle — the **same 149** it drew at GP 30 before the rescale. **⤳ RE-BASED 01-09-26 by the GP rescale — a RE-DENOMINATION, not a retune. See the AS-BUILT note below; do not read the change as a decision to make issuance meaner or more generous.** ⤳ **RESOLVED 02-09-26 (`fuel-supply-and-allocation.md §2.2`, corrected post-seed): the base stays contribution-relative and is a PRICE dial — at equilibrium total physical draw = influx and the base CANCELS from each guild's share (measured: 0.3→0.11 barely moved grants, moved the price 12→4.6). **Kept at 0.3** (yields ~reference price). The abundance/haul lever is `DEUTERIUM_INFLUX_PER_CYCLE`; the baseline was deliberately LEFT LOOSE on the new seed. §2.2 has the corrected picture.** |
| `DEUTERIUM_INFLUX_PER_CYCLE` | **800** | The abstracted back-end supply (§1.1). Fixed here, set to roughly balance a typical small galaxy's draw; a solo galaxy over-fills and a large one rations, because the influx is fixed and the draw is not — which is precisely what 5b's demand-relative controller corrects. Wants a proper `[SHEET]` pass with the price curve. **BUILT 31-08-26 (slice 5a)**, minted into the pool at each boundary and recorded in `audit.totalProduced`. ⤳ **02-09-26: this IS the abstracted PG baseline (§1.1) and the abundance/haul LEVER — the base only sets price. Left at 800 (deliberately loose on the new 96-waystation seed: ~32 baseline hauls for an on-line guild in a small galaxy; hauls-per-guild is population-relative, and the throttle emerges as a server fills). Grows organically as player deuterium production adds to the pool — no population formula. See `fuel-supply-and-allocation.md §2.2`.** |
