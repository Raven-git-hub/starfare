...
|---|---|---|
| `MEANLINE_K` | `[FIRST-CUT]` **150** *(slice 4)* | `expectedRP = MEANLINE_K × GP`. **Calibrated by model**, not guessed: at 150 a dense moderate-terms all-meeting guild sits ≈ on its line (modifier 1.0), a same-production sprawler is throttled (~0.33), a just-expanded guild pins at the floor, a developed big guild is mildly buffed. Large because RP rests in the thousands (taper cap ~1500/venture) while GP is in the tens — scale the GP weights ×10 if a rounder k is wanted. **BUILT 31-08-26 (slice 4)**, `sim/meanline.js` `expectedReputation`. **Calibration verified against the engine:** dense 1-system/3-venture guild **0.997**, the same three sprawled over three systems **0.330**, just-expanded **0.300** (floor), developed 2-system/8-venture guild **1.160**. "Par" is ≈ **three ventures per held system**. |
...
| `W_SYS` / `W_T1` / `W_T2` (GP weights) | `[FIRST-CUT]` **12 / 6 / 8** *(GP slice 3)* | `GP = W_SYS·(held systems) + Σ ventures W_TIER(tier)`, tier via `producedGoodFor` (raw→T1, processed→T2). Systems Points-heavy, ventures Points-light, refinery (T2) a notch above mine (T1). **Relative only** — slice 4's `MEANLINE_K` sets the absolute scale. Integer GP; **DERIVED**, never stored (state goldens untouched). `W_T3`/`W_T4` `[DEFERRED]` until those goods are real. **BUILT 31-08-26 (GP slice 3)**, `sim/points.js` `guildPoints`; the weights live there as `W_SYS` and a `TIER_WEIGHT` tier→weight map, so a new tier is one line. An unweighted tier **halts** rather than scoring 0. |
| Outpost deploy RP offset | `[DEFERRED]` | `= MEANLINE_K × (outpost GP)` (bar-neutral, `points-and-reputation.md §1.2`); when outposts are built. |

**Guild founding endowment — DERIVED, not a constant (ruled 31-08-26, A′; **BUILT 31-08-26**, `sim/meanline.js` `foundingEndowmentFor` off `sim/points.js` `systemPoints`; `docs/points-and-reputation.md §2.5`).** A one-time RP grant set at founding `= MEANLINE_K × (home-system GP)` `= 150 × 12 = 1800`, so a newborn opens on its line instead of pinned at the floor. It invents **no** number — a function of `MEANLINE_K` and `W_SYS`, both already ruled, and it tracks them if either retunes. **Not a tunable**; do not add it as a `[FIRST-CUT]`/`[SHEET]` row.
...
⚠ **`reserve.fuelPrice` HAS NO FLOOR YET, and that is 5b-ii's to give it.** It is the sanctioned float (§15.2) and the invariants sweep it for finiteness and non-negativity, but nothing bounds it away from **zero**, which the grant conversion divides by. 5b-i cannot reach that state — it seeds the price and never writes it — so the build closed the gap the only way that invents no number: `physicalGrantFor` **throws** on a price that is not finite and `> 0`. The `floor`/`ceiling` that make the controller respect that are two of the `[SHEET]` coefficients above, and they are not a first cut taken here.
