...
|---|---|---|
| **Base price** | **per manufacturing tier** — T1 **1**, T2 **10**, T3 **100** (RULED 26-09-26) | The seed value every good starts at and the anchor the curve multiplies, now **tier-scaled** so a processed part is not priced as if it were raw ore. Keyed by `tierOf` (`sim/points.js`); `deuterium` is tier 1 but never priced, so its base is moot. Supersedes the earlier uniform **10**. A ~10× step per tier is the value-ladder first cut (the human's numbers, not invented here); it is deliberately **decoupled from recipe input:output ratios** — see the refining-pump ruling, design.md §5 (Syndicate Exchange). |
