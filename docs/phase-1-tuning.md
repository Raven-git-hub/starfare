...
| `ISSUANCE_SENSITIVITY` | **1.5** *(slice 4)* | Slope on `gap/expected`. Raised 1.0→1.5 to lean **punishing for ambition** — a below-line guild falls faster. |
| `ISSUANCE_FLOOR` | **0.3** *(slice 4)* | A deeply below-line guild draws 0.3 of its `baseGrant` — no death spiral, but over-expansion stings. Lowered 0.5→0.3 (punishing ambition); asymmetric with the 1.5 CEIL so falling below the line hurts more than rising above helps. |
...
|---|---|---|
| `MEANLINE_K` | `[FIRST-CUT]` **150** *(slice 4)* | `expectedRP = MEANLINE_K × GP`. **Calibrated by model**, not guessed: at 150 a dense moderate-terms all-meeting guild sits ≈ on its line (modifier 1.0), a same-production sprawler is throttled (~0.33), a just-expanded guild pins at the floor, a developed big guild is mildly buffed. Large because RP rests in the thousands (taper cap ~1500/venture) while GP is in the tens — scale the GP weights ×10 if a rounder k is wanted. |
...
the snapshot. **Still NOT built:** the three `ISSUANCE_*` constants above (no consumer — the mean line is
unbuilt), `MEANLINE_K`, the outpost offset, and — importantly — the −500 / −300 **consequences**. Reaching the floor is a PIN: the venture keeps producing, keeps being judged and
