...
**Guild founding endowment — DERIVED, not a constant (ruled 31-08-26, A′; `docs/points-and-reputation.md §2.5`).** A one-time RP grant set at founding `= MEANLINE_K × (home-system GP)` `= 150 × 12 = 1800`, so a newborn opens on its line instead of pinned at the floor. It invents **no** number — a function of `MEANLINE_K` and `W_SYS`, both already ruled, and it tracks them if either retunes. **Not a tunable**; do not add it as a `[FIRST-CUT]`/`[SHEET]` row.
