#!/usr/bin/env bash
set -euo pipefail
# Starfare design-doc commit — run from the repo root:
#   gitserver@gitserver:/mnt/git-server/starfare
# Applies: TRADE mockup + design.md SELL ruling + README note, then commits.
# It does NOT push — review the diff, then run:  git push origin main
test -f docs/design.md || { echo "!! run me from the starfare repo root"; exit 1; }

# ---- 1. NEW: the TRADE tab mockup (visual+interaction contract) ----
  ...
  <div class="body">
    ...
    <div class="trade">
      ...
      <div class="card">
        ...
        <div class="syn-body">
          <div class="hold"><span>GUILD STOCKPILE</span><b>1,420&#8202;u alloy</b></div>
          <div class="qtyrow">
            <div class="qtybox">
              <input id="qty" type="text" value="500" inputmode="numeric" aria-label="Quantity to sell">
              <span class="u">u</span>
              <div class="step"><button id="up" aria-label="increase">&#9650;</button><button id="dn" aria-label="decrease">&#9660;</button></div>
            </div>
            <button class="maxbtn" id="max">Max</button>
          </div>
          <div class="forline"><span class="lbl">You receive</span><span class="amt"><span class="c">&#162;</span><span id="proceeds">11,050</span></span></div>
...
(function(){
  ...
  var qty=document.getElementById("qty"), proceeds=document.getElementById("proceeds"), HOLD=1420;
  function fmt(n){return Math.round(n).toLocaleString("en-US");}
  function recalc(){ var q=parseInt((qty.value+"").replace(/[^0-9]/g,""),10); if(isNaN(q))q=0; if(q>HOLD){q=HOLD; qty.value=q;} proceeds.textContent=fmt(q*NOW); }
  qty.addEventListener("input",recalc);
  document.getElementById("up").addEventListener("click",function(){ qty.value=(parseInt(qty.value.replace(/[^0-9]/g,""),10)||0)+50; recalc(); });
  document.getElementById("dn").addEventListener("click",function(){ qty.value=Math.max(0,(parseInt(qty.value.replace(/[^0-9]/g,""),10)||0)-50); recalc(); });
  document.getElementById("max").addEventListener("click",function(){ qty.value=HOLD; recalc(); });
  ...
  document.getElementById("sellBtn").addEventListener("click",function(){
    var q=parseInt(qty.value.replace(/[^0-9]/g,""),10)||0, self=this;
    self.textContent="Sold "+fmt(q)+"u  -  +c"+fmt(q*NOW); self.style.background="#5fb98a";
...
# ---- 1b. verify it landed byte-identical (fail loud) ----
echo "f34154cfda9e2910690499bbf56612306edb0bbf423b9eebd48adeb0fb0a28e2  docs/mockups/trade-panel.html" | sha256sum -c -

# ---- 2. EDIT design.md: append the SELL ruling ----
python3 - <<'PYEOF'
p = "docs/design.md"
s = open(p, encoding="utf-8").read()
anchor = "\n\n---\n\n## 6. Logistics: Travel Time, Vehicles, and Risk in Transit"
assert s.count(anchor) == 1, f"design.md anchor count={s.count(anchor)} (expected 1) - repo not in expected state"
block = r'''
...
'''
s2 = s.replace(anchor, "\n\n" + block.strip("\n") + anchor, 1)
assert s2 != s and s2.count("SELL GOES LIVE") == 1, "design.md insert failed"
open(p, "w", encoding="utf-8").write(s2)
print("design.md: SELL ruling appended")
PYEOF

# ---- 3. EDIT docs/mockups/README.md: note the mockup ----
python3 - <<'PYEOF'
p = "docs/mockups/README.md"
s = open(p, encoding="utf-8").read()
anchorA = r'''- **venture-establishment.html** — the establishment-popup design (the licence
  economy shown here is design-ahead / mock; the real wire is `establishVenture`).'''
assert s.count(anchorA) == 1, f"README anchorA count={s.count(anchorA)} (expected 1)"
s = s.replace(anchorA, anchorA + r'''
- **trade-panel.html** — the **TRADE tab** design (the Syndicate Exchange's first
  ...
  action to `/action`. Trader art is `client/assets/mission/Trader.jpg`.''', 1)
anchorB = "## Current state (25-08-26)"
assert s.count(anchorB) == 1, f"README anchorB count={s.count(anchorB)} (expected 1)"
s = s.replace(anchorB, anchorB + r'''
...
book are deferred (design.md §5, "SELL GOES LIVE").''', 1)
assert s.count("trade-panel.html") >= 2, "README insert failed"
open(p, "w", encoding="utf-8").write(s)
print("README: trade-panel.html noted")
PYEOF

# ---- 4. stage + commit (NO push) ----
git add docs/mockups/trade-panel.html docs/design.md docs/mockups/README.md
git commit -m "docs: TRADE panel mockup + Syndicate SELL ruling

Land the market tab's visual+interaction contract (docs/mockups/trade-panel.html)
and record the first executable Exchange slice in design.md section 5: SELL to the
Syndicate is guild-wide, drains largest-pile-first, executes immediately at the
posted price, credited round(qty*price) (decision #43) funded from the Syndicate
ledger (invariant 2 holds), no reserve guard, market impact via the existing
level-based pricing, fuel never sold. BUY and the guild-to-guild book stay deferred.
Doc-only: the code lands via Claude Code, which ticks the roadmap in its own commit."

echo; echo "Done. Review:  git show --stat HEAD   then push:  git push origin main"
