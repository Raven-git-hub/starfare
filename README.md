...
**Where the project actually is (05-08-26):** the economy runs as software and you can drive it from a browser. Phase 1's Stage 2 walking skeleton is well underway: a pure tick loop (`sim/run.js`'s `advance` = *intake → tick → assert*) with invariants 1–3 (conservation of fuel, conservation of credits, non-negativity/integrality) asserted every tick, determinism (9) checked across runs, and 4–8 landing with the systems they govern; guilds **founded** onto real seed homeworlds (starting credits **debited from the Syndicate ledger**, not minted); **mining** ventures on resource nodes; and **refining** ventures on settlement slots running a **multi-input recipe engine** — a recipe consumes several raw goods at once and is throttled by its scarcest input. The **Phase-2 (raw→processed) catalog** now holds 11 recipes (e.g. `titanium_alloy` ← 3 titanium + 1 carbon_products; `silicon_wafer` ← silica + silver + palladium), all with clearly-flagged `[PLACEHOLDER]` ratios awaiting a live tuner. The whole engine is wrapped in a thin dev-harness **server** (`sim/server.js`) and a **testbed web UI** (`client/testbed.html`) shipped in a **Docker container** on a self-hosted box (port 7331). The UI has grown a read-only **Guild Management tab**: pick a guild, drill **galaxy → system → planet**, and **establish ventures in place** (deploy a mine on a resource node, a refinery-with-recipe-picker on a settlement slot); the galactic-supply panel is split into the four manufacturing-tree tiers. The server exposes the engine over a small read surface (`/snapshot`, `/starters`, `/system/:id`, `/recipes`, `/goods`) plus `/tick`, `/action`, `/reset`, staying zero-game-logic. State is **in-memory + ephemeral** (a Reset, not persistence); **PostgreSQL is deferred to Phase 2**. In parallel, the galaxy generator is fully populated (archetypes, resource nodes, settlement slots, starter tagging, validation) and the seed viewer renders it with a System View drill-down. Resources are now **system-scoped** in code (ruling B1: a guild's stockpiles are a pool *per system*), and the Guild-Management tab has a read-only **Production view** — per system, per good, it lists producing and consuming ventures with per-tick output/demand and a contention summary, making the current first-come-first-served draw visible. Still ahead in Phase 1: the interactive **production flow** — sliders backed by a per-system **production profile** plus the allocation rewrite (design.md §5), then a **licence** pass; a **live recipe tuner** to set the placeholder ratios by playing; the **fuel market/pricing**; a **second (bot) guild** for real contention; and **tolls/shipping**. Two past artifacts (`galaxy-map-hex.html`, `SyndicateMarketplace.jsx`) remain unrecovered.

**Update — 24-08-26 (client-wiring, slices 0–1).** The dev rig is becoming the game (`docs/client-wiring.md`). The server now serves the committed seed at `GET /galaxy`, a **watch-only** operator panel at `GET /inspect` (full god's-eye state, zero controls), and starts its tick heartbeat on boot from `STARFARE_TICK_MS`. `GET /` is no longer the testbed: **`client/testbed.html` is deleted** and the front door is **the player client** (`client/game.html`), where you found your guild for real and explore the live 1,500-system galaxy with a HUD read from `/snapshot`. **The section completed 25-08-26** with slices 2–4: the system/planet drill-down reads live occupancy, ventures are established from the game through the ported establishment popup (its licence terms still mock over a real venture), and the Production Console is un-mocked — the client embeds the served `/console`, focused on the system you are viewing, so profiles, priorities and throttles tune the live persistent economy. The whole core loop — found → explore → establish → produce → persist — now runs through the client, with `/inspect` as the operator watcher.
...
│   └── tests/                   ← node --test; 169 green (invariants, determinism, refining, server…)
...
│   ├── game.html              ← THE PLAYER CLIENT the server serves at GET / (found · explore · produce)
│   ├── console.html           ← the player-facing Production Console, served at GET /console
│   └── inspect.html           ← the operator watch-only god's-eye panel, served at GET /inspect
...
|---|---|---|---|
| `docs/design.md` | The master design & technical document; the contract. Sections 1–20: vision, galaxy, economy/manufacturing tiers, ventures & the Exchange, logistics, politics, fuel, storyteller, bots, narrative layer, onboarding, architecture, data model & invariants, seed generation, roadmap summary, AI workflow, open questions | ✅ Current through 05-08-26 (last consolidated 19-07-26) | Ongoing upkeep in the same commit as the code it describes (incl. bumping the header date) |
| `docs/roadmap.md` | Authoritative phase-by-phase checklists and exit criteria | ✅ Current (05-08-26); Track G generator complete 02-08-26 | Boxes ticked as work lands; Phase 1 walking skeleton continuing |
...
| `client/game.html` | **THE PLAYER CLIENT**, served at `GET /` (client-wiring Slice 1, 24-08-26 — it replaced the deleted `testbed.html` as the front door). Splash → uplink → **found your guild** (pick a starter, name it, `foundGuild`) or adopt the galaxy's existing one; then the galaxy map + HUD on the real committed seed (`GET /galaxy`), territory from the live `claims[]`, and a ~1 s `/snapshot` poll. Reads the player-safe projection only; computes no game number | ✅ Works; found + adopt paths verified in headless Chromium (24-08-26) | Live occupancy on the detail screen (Slice 2), establish-a-venture (Slice 3), console un-mock (Slice 4) |
| `client/inspect.html` | The **operator watcher**, served at `GET /inspect` (client-wiring Slice 0): a ~1 s poll of `/snapshot` rendering the FULL god's-eye state — supply, ledger, every guild's stockpiles/profile, the production block, ventures, occupancy, claims. **Zero action controls**; never linked from the player client | ✅ Works | Real per-viewer server-side filtering (deferred, Phase 2/3) |
...
```bash
# Run the engine's test suite (169 tests: invariants, determinism, refining, server, …)
cd sim && node --test
